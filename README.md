...
| Suunto Training Guides | Reuses the existing Suunto OAuth credentials and `SUUNTOAPP_SUBSCRIPTION_KEY` with Guides access; non-secret `SUUNTOAPP_GUIDE_OWNER` (exact existing OAuth application name). See [Training delivery](docs/training-workspace.md#suuntoplus-guide-delivery-650). |
