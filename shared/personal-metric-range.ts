...
export const HRV_PERSONAL_RANGE_OPTIONS = Object.freeze({ baselineWindowDays: 60,
  baselineMinimumObservationDays: 14, currentWindowDays: 7, currentMinimumObservationDays: 3 } as const);
export const HRV_PERSONAL_RANGE_VARIANTS = ['overnight_rmssd', 'overnight_average',
  'sleep_overnight_hrv', 'sleep_session_average_hrv'] as const;
...
/** Daily baseline independent of current readings; optional observation instants align chart colors and shading. */
export function calculatePersonalMetricRangeTimeline(
  observations: readonly PersonalMetricRangeObservation[], startTimeMs: number, endTimeMs: number,
  options: PersonalMetricPointRangeOptions, observationTimes: readonly number[] = [],
): Array<{ timestampMs: number; normalRange: { min: number; max: number } | null }> {
  if (!Number.isFinite(startTimeMs) || !Number.isFinite(endTimeMs) || endTimeMs < startTimeMs
    || endTimeMs - startTimeMs > 366 * DAY_MS) return [];
  const times = new Set(observationTimes.filter(time => Number.isFinite(time) && time >= startTimeMs && time <= endTimeMs));
  for (let time = startTimeMs; time <= endTimeMs; time += DAY_MS) times.add(time);
  times.add(endTimeMs);
  return [...times].sort((a, b) => a - b).map(timestampMs => {
    const baseline = calculatePersonalMetricRange(observations, timestampMs, { ...options,
      currentWindowDays: options.baselineWindowDays, currentMinimumObservationDays: options.baselineMinimumObservationDays });
