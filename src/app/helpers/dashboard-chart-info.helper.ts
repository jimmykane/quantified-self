...
const DASHBOARD_CHART_INFO_COPY: Record<DashboardSpecialChartType, string> = {
  ...
  [DASHBOARD_SLEEP_TREND_CHART_TYPE]: 'Sleep Trend shows synced Garmin, Suunto, and COROS sessions separately by source. Stage stacks appear when the provider supplies them; otherwise total sleep is shown as unknown stage.',
  ...
  [DASHBOARD_LOAD_STATUS_KPI_CHART_TYPE]: 'Load Status combines same-day Form (TSB), CTL trend, and fatigue pressure into one current-state label. It is a dashboard summary, not a separate training model.',
  ...
  [DASHBOARD_FITNESS_CTL_KPI_CHART_TYPE]: 'Fitness (CTL) is chronic training load from the derived Form model, using a 42-day exponential moving average of daily TSS. Higher values indicate more accumulated fitness load.',
  [DASHBOARD_FATIGUE_ATL_KPI_CHART_TYPE]: 'Fatigue (ATL) is acute training load from the derived Form model, using a 7-day exponential moving average of daily TSS. Higher values indicate more recent training stress.',
  ...
  [DASHBOARD_RECOVERY_DEBT_KPI_CHART_TYPE]: 'Recovery Debt estimates how many zero-load days are needed for same-day Form (TSB) to return to neutral, using the same decay model as Freshness Forecast.',
