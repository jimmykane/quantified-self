...
const DASHBOARD_CHART_INFO_COPY: Record<DashboardSpecialChartType, string> = {
  ...
  [DASHBOARD_POWER_CURVE_CHART_TYPE]: 'Power Curve compares your best power per duration inside this tile range with the newest ride in range that has power-curve data. It uses the stored PowerCurve event stat, so the tile follows its own date and activity filters.',
