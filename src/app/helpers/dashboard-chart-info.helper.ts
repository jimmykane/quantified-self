...
const DASHBOARD_CHART_INFO_COPY: Record<DashboardSpecialChartType, string> = {
  ...
  [DASHBOARD_INTENSITY_DISTRIBUTION_CHART_TYPE]: 'Intensity Distribution groups weekly training into Easy (Z1-2), Moderate (Z3-4), and Hard (Z5-7). Each bar’s height is the total recorded zone time, while its colors show the percentage split, so a short hard session stays a short bar. Power zones are used first; heart-rate zones are the fallback when power is missing.',
  ...
  [DASHBOARD_HRV_TREND_CHART_TYPE]: 'HRV uses the same source-separated chart as Health, including recorded Sleep HRV. The date selector controls the period shown. For eligible nightly series, the shaded personal range and reading colors are calculated from the preceding 60 days of history for each date; this baseline is separate from the visible period. Missing readings remain gaps; naps are excluded. HRV has its own date range and navigation, independent of Sleep.',
  ...
  [DASHBOARD_LOAD_STATUS_KPI_CHART_TYPE]: 'Load Status combines current TSB, CTL ramp, current CTL, and current ATL into one current-state label. It is a dashboard summary, not a separate training model. It is TSS-only; recorded recovery signals appear separately in Today Readiness.',
  [DASHBOARD_FORM_NOW_KPI_CHART_TYPE]: 'Form Now is current TSB (CTL minus ATL) from your latest derived state, decayed through today with zero load when needed. Negative values indicate residual fatigue; positive values indicate fresher readiness. It is training-load only; Today Readiness is the separate score that can add recorded recovery signals.',
