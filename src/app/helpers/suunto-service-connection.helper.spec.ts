...
describe('suunto-service-connection.helper', () => {
  it('builds the default connected-state copy when reconnect is not required', () => {
    ...
    expect(result).toMatchObject({
      ...
      showDetails: true,
      description: 'Required for history imports and FIT activity uploads.',
      failureMessage: null,
      statusTone: 'default',
  ...
  it('builds reconnect-required state from service meta', () => {
    ...
    expect(result).toMatchObject({
      ...
      showDetails: true,
      description: 'Reconnect Suunto to resume sleep sync, history imports, and FIT uploads. If you use Garmin or COROS auto-sync into Suunto, re-enable those routes after reconnecting.',
