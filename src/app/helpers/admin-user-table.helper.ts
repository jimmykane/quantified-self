...
export interface AdminUserTableRow {
    ...
    canOpenSubscriptionGiftDialog: boolean;
...
export function buildAdminUserTableRows(users: readonly AdminUser[], locale: string): AdminUserTableRow[] {
    ...
    return users.map(user => {
        ...
        const isSubscriptionGiftEligible = !isAdmin
            && !user.disabled
            && (role === 'basic' || role === 'pro')
        ...
        return {
            ...
            role,
            isAdmin,
            isSubscriptionGiftEligible,
            canOpenSubscriptionGiftDialog: isSubscriptionGiftEligible
                || (!isAdmin && user.hasSubscribedOnce === true),
            subscriptionState,
