...
describe('Training delivery private rollout', () => {
  ...
  it('permits Garmin and Suunto only for the exact owner identity without enabling public delivery', () => {
    for (const provider of PLANNED_WORKOUT_PROVIDER_IDS) {
      expect(isPlannedWorkoutProviderDeliveryEnabled(provider)).toBe(false);
      expect(isTrainingProviderDeliveryEnabled(provider, pilotUid)).toBe(['garmin', 'suunto'].includes(provider));
    }
  });
