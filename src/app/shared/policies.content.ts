...
export const CONNECTED_SERVICES_POLICY_SECTION: ConnectedServicesPolicySection = {
    ...
    topics: [
        ...
        },
        {
            ...
            title: 'COROS Data',
            icon: 'sync',
            summary: 'COROS activity, daily Health and sleep, activity sync, FIT upload, and GPX/FIT or saved-route delivery workflows.',
            content: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Activity-type catalog:</strong> Any authorized MCP client can discover canonical Sports Lib activity types for route and activity filters. This static catalog contains no account data. <strong>Activity-detail permission:</strong> Individual activity access can return non-location summaries and parent event tags, laps, swim lengths, MTB jump measurements, selected persisted numeric metrics, signed-in application links, and bounded chart-ready streams. Activities from the same event share tags. It can filter bounded newest-first scans by activity type and resolve today or yesterday only with an explicit IANA timezone. A client can also read tags or filter by 1–10 exact case-insensitive tags using any/all semantics. Tag reads select only the event tag fields and exclude event names, descriptions, internal IDs, creator/source metadata, and coordinates. This uses the existing permission, so existing connections do not need broader consent, though a client may need to refresh its tool catalog; the built-in Assistant is not expanded. A chart request temporarily reads and selectively parses an existing original FIT, GPX, TCX, Suunto JSON/SML, or gzip file, downsamples the complete activity, discards parsed objects, and does not create a reparse, backfill, cache, or additional activity record. Historical charts depend on the original source remaining available and within processing limits.',
