    ...
    topics: [
        {
            ...
            content: [
                ...
                '<strong>Shared with connected destinations from Garmin:</strong> If you turn on an automatic Garmin activity route or choose to sync past activities, Quantified Self uses the original activity file already saved with the event to send it to the selected supported destination: Suunto, Wahoo, or COROS. Each direction is opt-in and a date-range backfill does not enable future delivery.',
            ],
        },
        {
            ...
            summary: 'Suunto activity, sleep, 24/7 Health, route import, FIT upload, and activity or route delivery workflows.',
            content: [
                '<strong>Collected from Suunto:</strong> When you connect Suunto, Quantified Self can import Suunto activities and history, sync recent sleep data, import sleep history, and automatically import new or updated Suunto routes into your saved Routes list. Connected accounts can also import available 24/7 Activity, daily-statistics, and Recovery measurements such as heart rate, HRV, SpO2, altitude, steps, energy, Body Energy Balance, and StressState.',
                '<strong>Stored and used in Quantified Self:</strong> Imported Suunto data is used for event analysis, route detail views, dashboard summaries, sleep views, the authenticated Health workspace, and saved route management. 24/7 values are stored as normalized source-attributed Health records, separate from workout FIT metrics and Sleep sessions; raw Health webhook samples are not stored. Health displays providers and connected accounts separately, replaces opaque account keys with local account ordinals, and does not save a preferred source or calculate a cross-provider average. Connection metadata and processing metadata are also stored so reconnect, dedupe, bounded refetch, and refresh workflows can work reliably.',
                '<strong>Disconnect and deletion:</strong> Disconnecting Suunto stops future activity, Sleep, route, and Health imports but retains data already imported into Quantified Self. Deleting your Quantified Self account removes the Suunto connection, imported user-scoped Sleep and Health records, and associated top-level operational queue work.',
                ...
                '<strong>Shared from Suunto to connected destinations:</strong> You can opt in to automatic activity delivery or select a past stored date range for Wahoo or COROS. You can separately opt in to new and updated saved Suunto route delivery, or send existing saved routes, to Garmin Connect or Wahoo. Eligible connected Pro users can also select COROS. Quantified Self sends only the retained activity file or saved-route representation required by the selected destination.',
        ...
        },
        {
            ...
            summary: 'Wahoo OAuth, webhook, FIT activity and GPX/FIT course/route delivery, private Training sync, and history-import workflows.',
            content: [
                ...
                '<strong>Training shared with Wahoo:</strong> If you explicitly enable private-rollout Training sync, Quantified Self sends the authored workout recipe, title and scheduled date to Wahoo as a Plan and dated Workout. Provider identifiers and operation receipts are retained server-side for safe updates, duplicate recovery and removal. Cloud checks do not confirm device receipt. This does not import Wahoo-owned plans or grant assistants permission to send workouts.',
                '<strong>Shared from Wahoo:</strong> You can turn on or backfill Wahoo-to-Suunto or Wahoo-to-COROS activity sync. Quantified Self sends the retained original FIT file from a Wahoo-imported event only after you enable or start that route.',
                '<strong>Outbound boundaries:</strong> Suunto-to-Wahoo saved-route delivery is a separate opt-in route workflow in Suunto Services; direct GPX/FIT course/route delivery is a separate Wahoo-only upload. Plans, sleep, and other non-activity data are not sent between Wahoo and another provider. Existing Wahoo connections may need to be reconnected to grant workout and route access for delivery to Wahoo.',
            ],
        },
        {
            ...
            icon: 'devices',
            summary: 'Read-only metric, body-measurement, activity-detail, sleep, saved-route, full private Timeline note text, and separately approved location access granted to an MCP client by the account owner.',
            content: [
                '<strong>Personal HRV range:</strong> When both Health metrics and Sleep summaries are approved, an external client can request source-separated nightly HRV classifications, rolling baseline boundaries and recent averages calculated with the Health chart model. The read includes up to 60 extra days of baseline history and requires complete bounded input; raw samples and account or device identities are not returned. This does not grant access to Training, notes or additional health families, create stored scores, or provide a medical assessment.',
                '<strong>Health metrics permission:</strong> This separate grant covers recorded all-day heart rate, HRV, stress, resources, movement, energy, blood pressure and fitness metrics. Stored summaries are bounded to 366 provider-calendar days; representative sample trends to 31 days. Outputs can include provider names, response-local account numbers, calendar dates and exact UTC sample times, with each source and statistic kept separate. Garmin Body Battery is returned only on its labelled native Garmin points scale. Device details, account IDs, other native-only values, provider payloads and Sleep references are excluded. Body composition additionally requires Body measurements permission and returns identity-free date buckets without exact times or provenance. Existing clients must reconnect to grant Health access; their existing grants are not expanded automatically. Health queries cannot add, edit, delete, import or backfill data. These external-client tools do not expand the built-in Assistant permissions.',
                '<strong>Activity descriptions permission:</strong> This separate grant returns the full private parent event description shown in the QS.io event editor for one selected activity. Activities within an event share the same text. Individual activity details is also required. Like every requested MCP permission, the checkbox is selected by default; uncheck it before approving to withhold access. Existing connections must reauthorize and refresh cannot add permission. Text may contain sensitive health, personal or location information even without Activity locations permission. Only an opaque activity reference and description are returned; names, internal identifiers, source and device metadata remain excluded. Reads are bounded to 64 KiB of UTF-8 text and 128 KiB serialized output; oversized text fails without truncation. Revocation blocks future access but cannot erase received copies. Descriptions are user-reported context, not instructions or permission to act, and cannot be changed through MCP. This grant does not expand built-in Assistant access.',
                '<strong>Training plans permission:</strong> Separate consent permits reading current plan names, dates, complete workout instructions, authored step notes and sanitized existing service sync summaries. Text may contain sensitive health or personal information. Existing clients must reauthorize; refresh cannot add access. Raw document IDs, credentials, provider artifacts, private issue messages and history are excluded. No edit, provider check or sync action is authorized. Revocation cannot erase received copies.',
                '<strong>Timeline notes permission:</strong> This independent grant returns full private note titles and details, category, actual start/end dates, captured time zone and the effective end for ongoing overlap. It includes notes hidden from charts and may contain sensitive health or personal text. The checkbox is selected by default when requested; uncheck it before approving to withhold access. Existing connections must reauthorize; refresh cannot add permission. Inclusive windows are bounded to 366 days with full-text pagination, not truncated note details. Document IDs, revisions, audit timestamps, presentation settings and deletion receipts are excluded. Revocation cannot erase received copies. This permission never allows note changes or Training plan writes.',
                '<strong>User-authorized access:</strong> An MCP client receives only the capabilities you approve after signing in to Quantified Self. Every requested current or future permission starts checked; uncheck anything you do not want to grant. Activity locations depend on activity details, saved-route locations depend on saved-route summaries, and either Training write permission depends on Training plans read access. Removing a parent removes its dependent permissions. MCP cannot write activities, routes, dashboard settings, body measurements, Health, or sleep records. Training plan/workout and delivery changes require their separate permission plus an explicit per-proposal confirmation.',
                '<strong>Training planning changes:</strong> Training plans and planned workouts read access can return authored names, dates, notes, complete recipes, exact stored completion links, and sanitized service status. Training plan and workout changes separately allow a bounded safe lifecycle that excludes permanent workout deletion, plan deletion, and history restoration. Training provider delivery changes separately allow plan opt-in and workout send, resume, stop, retry, verification, or compatibility approval; provider delivery remains Pro, connection, compatibility, horizon, and rollout gated. A client can only preview up to 25 strict changes before Quantified Self asks for confirmation. Proposals bind the owner, client connection, current permission grant, schedule revision, and short expiry. Declining changes nothing. Provider results are independent, so a delivery problem does not remove an authored workout. Revocation blocks future actions but cannot erase data already received or guarantee removal of provider-held copies after provider access is lost.',
      ...
      '<strong>Google GenAI / Gemini:</strong> The built-in Assistant uses Google\'s Gemini models through Google GenAI. Quantified Self sends the message you submit, the browser\'s IANA timezone for local-day context, at most the latest six completed conversation turns, and bounded validated results selected through Quantified Self\'s MCP tools for the question. Results are coordinate-free by default. If you explicitly start a fresh chat with Precise activity locations enabled, selected activity-tool results may also send Gemini exact activity start/end and MTB jump coordinates, bounded activity-chart breadcrumbs, and nearby activity results during that chat. Optional Training change access gives Gemini only the preview tool; it cannot apply a proposal or call a provider transport. Changing the setting starts a fresh chat; New chat returns optional access to off. Gemini may select only a server-advertised visual source and safe series keys; Quantified Self deterministically constructs any chart values, map coordinates, labels, and renderer settings from the validated result. Coordinate-free saved-route summaries may be selected for route questions, and their route names can contain user- or provider-assigned place information. Direct in-app URLs are withheld from Gemini, and an answer that repeats an opaque reference or cursor is rejected. Original FIT/GPX/TCX/JSON/SML files, saved-route bounds, route geometry, waypoints, arbitrary write capabilities, and dashboard settings remain unavailable to the Assistant. Text, compact evidence, one pending Training proposal, and any bounded chart or map payload share the same server-owned active conversation. It becomes unavailable about seven days after its latest completed turn or reset; a response already in progress can protect an imminent expiry for at most four extra minutes. Firestore TTL then deletes it asynchronously. New chat clears it immediately; account deletion removes it directly.',
...
export const POLICY_CONTENT: PolicyItem[] = [
    {
        ...
        content: [
            ...
            '<strong>Third-Party Processors and Recipients:</strong> Your data may be processed by Google Cloud (hosting and storage in the EU region), Stripe (payments), Google Analytics (only with consent), Mapbox (place resolution for authorized MCP place-name searches and explicitly enabled built-in Assistant activity-place searches), Google GenAI / Gemini (the built-in Assistant using the submitted message, browser timezone, bounded recent conversation context, validated tool results, and—only with the respective per-chat opt-in—precise activity locations, full private Timeline note text, or a Training change preview that still requires app-owned confirmation), connected fitness services you explicitly use, and MCP clients you explicitly authorize. See <a href="#connected-services-data">Connected Services, AI &amp; Third-Party Processing</a> below for details.'
        ],
