...
export const CONNECTED_SERVICES_POLICY_SECTION: ConnectedServicesPolicySection = {
    ...
    content: [
        '<strong>Training workout delivery:</strong> Manual plans and standalone workouts are stored beneath your account. Workout delivery to Garmin, COROS, Suunto, and Wahoo is available to connected Pro users. Every destination requires explicit opt-in. Delivery sends the workout recipe, title and scheduled date in your saved delivery time zone to the selected connected provider. Quantified Self retains delivery preferences, compatibility approvals and private operation/artifact records so edits, retries and removal can be reconciled safely. Use Stop sync before disconnecting to request eligible future-copy removal. Explicit disconnect invalidates delivery consent but may leave provider-held copies; Pro expiry preserves preferences and copies while pausing changes. Account deletion fences new delivery and recursively removes local delivery records and jobs, but cannot guarantee removal of copies already held by a provider after access is revoked.',
    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Shared back to COROS:</strong> You can send a selected FIT activity, automatically send new Garmin/Suunto/Wahoo FIT activities, or send a selected past date range already stored in Quantified Self. Eligible connected Pro users can also send a selected GPX/FIT route, a saved route from Routes, or opt in to new/updated or existing saved Suunto route delivery. Training sync can send a standalone workout to the COROS training calendar. New COROS plan-sync setup is currently labelled Coming soon in the app; existing saved plan opt-ins may continue to send the workout recipe, title and scheduled date. Delivery records, copy status, retry, per-workout exclusion and Stop sync remain available. COROS app/watch synchronization is controlled by COROS and a provider acceptance is not a watch receipt. Selected route files are parsed and converted to GPX in memory and do not create a Quantified Self route; saved routes retain provider delivery metadata for deduplication and status.',
