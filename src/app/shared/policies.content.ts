    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>User-authorized access:</strong> An MCP client receives data only after you sign in to Quantified Self and approve one or more requested read-only permissions. Every requested current or future permission starts checked; uncheck anything you do not want to grant before approving. Activity locations depend on activity details, and saved-route locations depend on saved-route summaries. Removing activity details also removes its descriptions and location permissions; removing saved-route summaries removes its location permission. The client cannot use MCP to write activities, routes, settings, Training state, body measurements, or sleep records.',
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Optional Assistant Training plans:</strong> Default-off access is independent of other permissions. Changing it creates a fresh server-owned chat while preserving other choices; New chat resets all optional access. Gemini may receive relevant instructions and sensitive authored names/notes, treated as untrusted context, never authority. Evidence stays compact; answers may quote relevant text under existing retention. No editing or sync actions are available.',
...
export const POLICY_CONTENT: PolicyItem[] = [
    {
        ...
        content: [
            ...
            '<strong>Third-Party Processors and Recipients:</strong> Your data may be processed by Google Cloud (hosting and storage in the EU region), Stripe (payments), Google Analytics (only with consent), Mapbox (place resolution for authorized MCP place-name searches and explicitly enabled built-in Assistant activity-place searches), Google GenAI / Gemini (the built-in Assistant using the submitted message, browser timezone, bounded recent conversation context, validated read-only results, and—only with the respective per-chat opt-in—precise activity locations or full private Timeline note text), connected fitness services you explicitly use, and MCP clients you explicitly authorize. See <a href="#connected-services-data">Connected Services, AI &amp; Third-Party Processing</a> below for details.'
        ],
