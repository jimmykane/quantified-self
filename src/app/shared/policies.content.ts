    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Shared back to COROS:</strong> You can send a selected FIT activity, automatically send new Garmin/Suunto/Wahoo FIT activities, or send a selected past date range already stored in Quantified Self. Eligible connected Pro users can also send a selected GPX/FIT route, a saved route from Routes, or opt in to new/updated or existing saved Suunto route delivery. Private-rollout Training sync can send a standalone workout, while an existing enabled plan setting continues to send the workout recipe, title and scheduled date to the COROS training calendar; new browser plan-sync setup is temporarily unavailable and shown as coming soon. Existing delivery records, copy status and Stop sync remain available. COROS app/watch synchronization is controlled by COROS and a provider acceptance is not a watch receipt. Selected route files are parsed and converted to GPX in memory and do not create a Quantified Self route; saved routes retain provider delivery metadata for deduplication and status.',
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>User-authorized access:</strong> An MCP client receives only the capabilities you approve after signing in to Quantified Self. Every requested current or future permission starts checked; uncheck anything you do not want to grant. Activity locations depend on activity details, saved-route locations depend on saved-route summaries, and either Training write permission depends on Training plans read access. Removing a parent removes its dependent permissions. MCP cannot write activities, routes, dashboard settings, body measurements, Health, or sleep records. Training plan/workout and delivery changes require their separate permission, a bounded preview, and a separate apply tool governed by the MCP host\'s approval controls.',
