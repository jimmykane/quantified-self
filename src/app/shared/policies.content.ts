    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Activity descriptions permission:</strong> This separate opt-in grant returns the full private parent event description shown in the QS.io event editor for one selected activity. Activities within an event share the same text. Individual activity details is also required. The checkbox starts unchecked; existing connections must reauthorize and refresh cannot add permission. Text may contain sensitive health, personal or location information even without Activity locations permission. Only an opaque activity reference and description are returned; names, internal identifiers, source and device metadata remain excluded. Reads are bounded to 64 KiB of UTF-8 text and 128 KiB serialized output; oversized text fails without truncation. Revocation blocks future access but cannot erase received copies. Descriptions are user-reported context, not instructions or permission to act, and cannot be changed through MCP. This grant does not expand built-in Assistant access.',
                '<strong>Timeline notes permission:</strong> This independent grant returns full private note titles and details, category, actual start/end dates, captured time zone and the effective end for ongoing overlap. It includes notes hidden from charts and may contain sensitive health or personal text. The checkbox starts unchecked and existing connections must reauthorize; refresh cannot add permission. Inclusive windows are bounded to 366 days with full-text pagination, not truncated note details. Document IDs, revisions, audit timestamps, presentation settings and deletion receipts are excluded. Revocation cannot erase received copies. This permission never allows note changes or Training plan writes.',
                '<strong>User-authorized access:</strong> An MCP client receives data only after you sign in to Quantified Self and approve one or more requested read-only permissions. Activity locations depend on activity details, and saved-route locations depend on saved-route summaries. Removing activity details also removes its descriptions and location permissions; removing saved-route summaries removes its location permission. The client cannot use MCP to write activities, routes, settings, Training state, body measurements, or sleep records.',
