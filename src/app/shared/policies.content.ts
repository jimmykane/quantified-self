    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Credentials and retention:</strong> MCP bearer and refresh credentials are opaque, stored server-side only as hashes, expire automatically, and are bound to your account and the MCP resource. Approving a request creates pending connection metadata, but the connection becomes active and appears in Settings only after the client successfully exchanges its authorization code. Abandoned pending approvals expire automatically. Authorization metadata and active connection metadata are retained so the connection can operate and be audited.',
