...
export const CONNECTED_SERVICES_POLICY_SECTION: ConnectedServicesPolicySection = {
    ...
    topics: [
        ...
        },
        {
            ...
            icon: 'devices',
            summary: 'Read-only metric, activity-detail, sleep, and saved-route access granted to an MCP client by the account owner.',
            content: [
                '<strong>User-authorized access:</strong> An MCP client receives data only after you sign in to Quantified Self and approve one or more requested read-only permissions. Existing metric or sleep grants do not automatically gain activity-detail or saved-route access. The client cannot use MCP to write activities, routes, settings, Training state, or sleep records.',
                ...
                '<strong>Activity-detail permission:</strong> Individual activity access can return bounded activity summaries with exact start and end latitude/longitude coordinates when available, laps, swim lengths, MTB jump measurements, and direct links containing stable account/event paths that still require your normal Quantified Self sign-in. Jump records can also include their exact latitude and longitude. Exact activity and jump coordinates can reveal your home, workplace, frequent trailhead, or other sensitive locations. Raw streams, activity names and notes, separate internal identifier fields, original files, device/provider provenance, and arbitrary stored stats are excluded.',
                ...
                '<strong>Saved-route permission:</strong> Saved-route access can return route names, bounded metrics and counts, exact geographic bounds, simplified polyline preview geometry, parsed waypoint coordinates, and direct links containing stable account/route paths that still require your normal Quantified Self sign-in. Original route files, raw route points and streams, waypoint names and comments, Storage paths, provider provenance, links, extensions, and delivery metadata are excluded.',
