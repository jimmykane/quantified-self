...
export const CONNECTED_SERVICES_POLICY_SECTION: ConnectedServicesPolicySection = {
    ...
    content: [
        ...
        '<strong>AI scope:</strong> Connected-service data is not forwarded wholesale to AI providers. The built-in Assistant sends Gemini the message you submit, the browser\'s IANA timezone for local-day context, bounded recent conversation context, and bounded validated coordinate-free results selected through Quantified Self\'s read-only MCP tools. Coordinate-free saved-route summaries may be selected for route questions; their route names can contain user- or provider-assigned place information. Direct in-app URLs are withheld from Gemini, and an answer that repeats an opaque reference or cursor is rejected. Coordinate fields, saved bounds, route geometry, waypoints, write capabilities, dashboard settings, and original uploaded source files are unavailable to the Assistant.',
    ],
    navLinks: [
    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Mapbox:</strong> When an authorized MCP client searches by place name, Mapbox is used to resolve the supplied place text and geographic scope. Direct-coordinate MCP searches do not call Mapbox. The built-in Assistant has no location-search tools and does not use Mapbox; its saved-route access is limited to coordinate-free summaries.',
                '<strong>Google GenAI / Gemini:</strong> The built-in Assistant uses Google\'s Gemini models through Google GenAI. Quantified Self sends the message you submit, the browser\'s IANA timezone for local-day context, at most the latest six completed conversation turns, and bounded validated coordinate-free results from the read-only tools chosen for the question. Coordinate-free saved-route summaries may be selected for route questions; their route names can contain user- or provider-assigned place information. Direct in-app URLs are withheld from Gemini, and an answer that repeats an opaque reference or cursor is rejected. Original FIT/GPX/TCX/JSON/SML files, coordinate fields, saved bounds, route geometry, waypoints, write capabilities, and dashboard settings are unavailable to the Assistant. One server-owned active conversation becomes unavailable about seven days after its latest completed turn or reset; a response already in progress can protect an imminent expiry for at most four extra minutes. Firestore TTL then deletes it asynchronously. New chat clears it immediately; account deletion removes it directly.',
...
export const POLICY_CONTENT: PolicyItem[] = [
    {
        ...
        content: [
            ...
            '<strong>Third-Party Processors and Recipients:</strong> Your data may be processed by Google Cloud (hosting and storage in the EU region), Stripe (payments), Google Analytics (only with consent), Mapbox (place resolution for MCP place-name searches), Google GenAI / Gemini (the built-in Assistant using the submitted message, browser timezone, bounded recent conversation context, and validated coordinate-free read-only results, including a saved-route summary when selected for the question), connected fitness services you explicitly use, and MCP clients you explicitly authorize. See <a href="#connected-services-data">Connected Services, AI &amp; Third-Party Processing</a> below for details.'
        ],
