...
export const CONNECTED_SERVICES_POLICY_SECTION: ConnectedServicesPolicySection = {
    ...
    content: [
        ...
        '<strong>Workout context in Health:</strong> When you select Weight or VO₂ max in the authenticated Health workspace, Quantified Self can read those values on demand from workouts already imported into your account. Workout Weight is labelled as profile context rather than a weigh-in and is hidden for a source when that source has a real Health Weight measurement in the active filtered window; a manual Weight measurement hides workout fallback across the view. Workout VO₂ max remains separate by provider, local account label, and discipline and is not merged with provider Health or manual values. These reads do not copy workout values into Health storage, and their bounded response excludes workout identifiers, names, locations, provider account IDs, and raw creator details.',
