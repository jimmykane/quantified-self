...
export const CONNECTED_SERVICES_POLICY_SECTION: ConnectedServicesPolicySection = {
    ...
    content: [
        ...
        '<strong>AI scope:</strong> Connected-service data is not forwarded wholesale to AI providers. The built-in Assistant sends Gemini the message you submit, the browser\'s IANA timezone for local-day context, bounded recent conversation context, and bounded validated non-location results selected through Quantified Self\'s read-only MCP tools. Direct in-app URLs are withheld from Gemini, and an answer that repeats an opaque reference or cursor is rejected. Saved routes, exact locations, write capabilities, dashboard settings, and original uploaded source files are unavailable to the Assistant.',
    ],
    navLinks: [
    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Mapbox:</strong> When an authorized MCP client searches by place name, Mapbox is used to resolve the supplied place text and geographic scope. Direct-coordinate MCP searches do not call Mapbox. The built-in Assistant has no location or saved-route tools and does not use Mapbox. The older AI Insights endpoint retained temporarily for rollback can still use Mapbox when an authenticated user submits a location-based request through that endpoint.',
                '<strong>Google GenAI / Gemini:</strong> The built-in Assistant uses Google\'s Gemini models through Google GenAI. Quantified Self sends the message you submit, the browser\'s IANA timezone for local-day context, at most the latest six completed conversation turns, and bounded validated results from the non-location read-only tools chosen for the question. Direct in-app URLs are withheld from Gemini, and an answer that repeats an opaque reference or cursor is rejected. Original FIT/GPX/TCX/JSON/SML files, saved routes, exact locations, write capabilities, and dashboard settings are unavailable to the Assistant. One server-owned active conversation becomes unavailable about seven days after its latest completed turn or reset; a response already in progress can protect an imminent expiry for at most four extra minutes. Firestore TTL then deletes it asynchronously. New chat clears it immediately; account deletion removes it directly. The older AI Insights endpoint retained temporarily for rollback continues to use its separately bounded derived-statistics flow until that endpoint is retired.',
...
export const POLICY_CONTENT: PolicyItem[] = [
    {
        ...
        content: [
            ...
            '<strong>Third-Party Processors and Recipients:</strong> Your data may be processed by Google Cloud (hosting and storage in the EU region), Stripe (payments), Google Analytics (only with consent), Mapbox (place resolution for MCP place-name searches and the temporarily retained legacy AI Insights rollback endpoint), Google GenAI / Gemini (the built-in Assistant using the submitted message, browser timezone, bounded recent conversation context, and validated non-location read-only results, plus the separately bounded legacy AI Insights flow while retained), connected fitness services you explicitly use, and MCP clients you explicitly authorize. See <a href="#connected-services-data">Connected Services, AI &amp; Third-Party Processing</a> below for details.'
        ],
