    ...
    topics: [
        {
            ...
            title: 'Garmin Data',
            icon: 'sync_alt',
            summary: 'Garmin activity, Sleep, staged Health, route delivery, and provider sync workflows.',
            content: [
                '<strong>Collected from Garmin:</strong> When you connect Garmin, Quantified Self can import Garmin activities, request activity and Sleep history, and receive Garmin Sleep updates. During the limited Garmin Health rollout, eligible accounts can also import available Daily, Stress Details, HRV, User Metrics, Body Composition, Pulse Ox, All-day Respiration, Blood Pressure, Skin Temperature, and Health Snapshot summaries.',
                '<strong>Stored and used in Quantified Self:</strong> Imported Garmin data is used for dashboard, event analysis, Sleep views, and related summaries. Staged wellness measurements are stored as normalized source-attributed Health records and bounded sample chunks, separate from workout metrics and Sleep sessions; missing values remain missing and Garmin Body Battery remains provider-specific. Short-lived Garmin pull callback URLs are retained only on retryable live queue work, removed after every terminal queue outcome, and excluded from failed-job copies. Quantified Self may retain original activity files or equivalent source-file metadata when downloads, exports, reprocessing, or syncing past activities require them.',
                '<strong>Disconnect and deletion:</strong> Disconnecting Garmin stops future activity, Sleep, and staged Health imports but retains data already imported into Quantified Self. Deleting your Quantified Self account removes the Garmin connection, imported user-scoped Sleep and Health records, sample chunks, sync state, and associated operational queue work.',
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Stored and used in Quantified Self:</strong> Imported Suunto data is used for event analysis, route detail views, dashboard summaries, sleep views, and saved route management. 24/7 values are stored as normalized source-attributed Health records, separate from workout FIT metrics and Sleep sessions; raw Health webhook samples are not stored. Connection metadata and processing metadata are also stored so reconnect, dedupe, bounded refetch, and refresh workflows can work reliably.',
