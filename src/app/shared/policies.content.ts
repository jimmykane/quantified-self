...
export const CONNECTED_SERVICES_POLICY_SECTION: ConnectedServicesPolicySection = {
    ...
    content: [
        ...
        '<strong>AI scope:</strong> Connected-service data is not forwarded wholesale to AI providers. The built-in Assistant sends Gemini the message you submit, the browser\'s IANA timezone for local-day context, bounded recent conversation context, and bounded validated results selected through Quantified Self\'s read-only MCP tools. The Assistant is coordinate-free by default. If you explicitly start a fresh chat with Precise activity locations enabled, selected activity-tool results may also send Gemini exact activity start/end and MTB jump coordinates plus nearby activity results during that chat. Changing this setting starts another fresh chat, and New chat returns it to off. Coordinate-free saved-route summaries may be selected for route questions; their route names can contain user- or provider-assigned place information. Direct in-app URLs are withheld from Gemini, and an answer that repeats an opaque reference or cursor is rejected. Saved-route bounds, route geometry, waypoints, write capabilities, dashboard settings, and original uploaded source files remain unavailable to the Assistant.',
    ],
    navLinks: [
