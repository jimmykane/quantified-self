    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Metric and body-measurement permission:</strong> This access can return numeric metrics already stored for your activities, bounded body-measurement history, and ready server-derived Training snapshots. Body-weight history is returned only as identity-free day, week, or month values for a range of at most 366 days; exact source measurement timestamps, event/activity identity, names, provider/device metadata, and source provenance are excluded. When individual activity access is also granted, a client can request up to 25 explicitly selected canonical numeric Sports Lib metrics for one referenced activity. Quantified Self excludes precise latitude/longitude metrics and removes event/activity identifiers, names, labels, source fingerprints, and imported device/provider source keys from Training payloads.',
