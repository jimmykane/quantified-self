    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Shared back to COROS:</strong> You can send a selected FIT activity, automatically send new Garmin/Suunto/Wahoo FIT activities, or send a selected past date range already stored in Quantified Self. Eligible connected Pro users can also send a selected GPX/FIT route, a saved route from Routes, or opt in to new/updated or existing saved Suunto route delivery. Training sync can send a standalone workout or, after explicit plan opt-in, send the workout recipe, title and scheduled date to the COROS training calendar. Delivery records, copy status, retry, per-workout exclusion and Stop sync remain available. COROS app/watch synchronization is controlled by COROS and a provider acceptance is not a watch receipt. Selected route files are parsed and converted to GPX in memory and do not create a Quantified Self route; saved routes retain provider delivery metadata for deduplication and status.',
