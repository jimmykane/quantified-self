...
export const CONNECTED_SERVICES_POLICY_SECTION: ConnectedServicesPolicySection = {
    ...
    content: [
        '<strong>Training workout delivery:</strong> Manual plans and standalone workouts are stored beneath your account. Wahoo workout delivery is available to connected Pro users; other provider delivery remains limited rollout. Every destination requires explicit opt-in. Delivery sends the workout recipe, title and scheduled date in your saved delivery time zone to the selected connected provider. Quantified Self retains delivery preferences, compatibility approvals and private operation/artifact records so edits, retries and removal can be reconciled safely. Use Stop sync before disconnecting to request eligible future-copy removal. Explicit disconnect invalidates delivery consent but may leave provider-held copies; Pro expiry preserves preferences and copies while pausing changes. Account deletion fences new delivery and recursively removes local delivery records and jobs, but cannot guarantee removal of copies already held by a provider after access is revoked.',
    ...
    topics: [
        ...
        },
        {
            ...
            summary: 'Wahoo OAuth, webhook, FIT activity and GPX/FIT course/route delivery, Training workout delivery, and history-import workflows.',
            content: [
