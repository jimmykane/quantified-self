    ...
    topics: [
        ...
        },
        {
            ...
            content: [
                ...
                '<strong>Saved-route summary permission:</strong> Saved-route access can return route names, activity types, bounded metrics, route/waypoint/point counts, import/update times, and signed-in application links. It omits exact bounds, preview geometry, and waypoint locations.',
                ...
                '<strong>Control and destination:</strong> Review or revoke MCP clients under Connections -> MCP. A client can use the standard server-to-server token-revocation endpoint, but it may not notify Quantified Self when removed or uninstalled. Disconnect in Connections remains the authoritative control and immediately invalidates that connection\'s access and refresh-token family without affecting other MCP clients. Account deletion removes MCP connection and authorization state. A client may retain data it already received according to its own privacy and retention practices, so authorize only clients you trust.',
            ],
        },
        {
