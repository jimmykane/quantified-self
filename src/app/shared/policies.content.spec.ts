...
describe('Built-in Assistant policy', () => {
  it('documents the bounded MCP-backed Gemini context, exclusions, and retention', () => {
    ...
    expect(content).toContain('bounded validated coordinate-free results from the read-only tools');
    ...
    expect(content).toContain('coordinate fields, saved bounds, route geometry, waypoints');
    ...
    expect(content).toContain('built-in Assistant has no location-search tools');
    expect(content).toContain('saved-route access is limited to coordinate-free summaries');
