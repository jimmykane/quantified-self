...
- Search within the section, use **Load**, **Readiness**, or **Execution** to filter KPIs, and scroll the compact chart list. Each row includes a small chart preview and a **Your data** or **Example data** label. If a filter excludes the previewed chart, select a matching row before adding.
- The section’s add action opens a chart picker: a wide dialog on desktop or a bottom sheet on mobile. Select a chart row to see its large preview, explanation, data scope, and destination section. Desktop opens with the first available chart previewed and keeps the list beside it; mobile opens the list first and uses **Back to charts** to return after selecting a chart.
