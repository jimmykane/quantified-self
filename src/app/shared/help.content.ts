...
- **COROS workout delivery is coming soon in the app.** New plan sync, plan-workout resume, and standalone Send actions are unavailable. Existing saved COROS delivery state continues to follow its consent; delivery records, retry, copy status, and Stop sync remain available. COROS plan delivery uses individual workouts in the COROS training calendar rather than creating or claiming a native QS-owned COROS plan. [COROS documents a two-week watch window and one training plan synced to a watch at a time](https://support.coros.com/hc/en-us/articles/360048955151-Creating-and-Using-Training-Plans). COROS does not provide a planned-workout read/list operation, so remote checking and automatic missing-copy restoration are unavailable.
...
- Workout delivery to Garmin, Suunto, and Wahoo is available to connected Pro members. New COROS plan sync and standalone Send actions are coming soon in the app. Every available destination remains off until the user explicitly enables plan sync or sends a standalone workout. Provider-specific compatibility, permissions, scheduling windows, and connection checks still apply.
- Wahoo supports time-based running and cycling workouts for today and the next six days in the saved time zone. Later dates wait automatically. Distance-based steps are not sent because Wahoo needs a total duration. Older Wahoo connections may need **Reconnect Wahoo** to grant Plan and Workout access; activity imports remain usable before that reconnect. The editor is unchanged. Bike computers use the first target in an interval; review any warnings before approving. Sent is cloud delivery, not a device receipt. Checks confirm the app-owned Plan, Workout and association; automatic missing-copy restoration is unavailable.
- If a standalone workout uses a sport or step Wahoo cannot deliver, the first sync review explains why and **Send workout** stays disabled. You can keep the workout in QS, edit it to a supported running/cycling time-based recipe, or send it to another compatible service. An earlier Wahoo copy, if any, may remain unchanged. Mapping adjustments that Wahoo can make are shown separately for your approval.
...
- New dashboards start with a full-width **Activity Calendar** tile showing the current month. Select a date to preview its activities, notes, and plans beside the calendar or below it when the tile is narrow. Scroll the day preview to see more without changing the calendar's height. The preview shows the first two entries in each group and counts the rest. Earlier dates also show their available health readings; today's health stays in the Today summary above. **Full day** opens a dedicated page with every entry and previous/next day controls.
- The **Today** card opens a month calendar in a bottom sheet. Use its arrows to change months and select a day for details, then choose **Open full day** for the complete day view. Browser Back reopens that day's sheet. The Dashboard and Training headers also link to the full [Calendar](/calendar).
...
- The selected day shows date-matched sleep and HRV with their sources. The full day page also shows the same sleep-stage breakdown used in Health when the selected night includes stages. Its timeline places notes and planned workouts without invented times, then overnight sleep and completed activities by their recorded times. Select sleep to jump to its stages, or an activity or note for details. Activity mix remains below. A short **What stands out** summary appears only when there is a relevant note or an earlier planned workout without a linked activity. On a phone, the date controls stay visible as you scroll; use **Calendar** above the date to return to the selected month. Sleep and HRV can appear while readiness is still loading. Readiness appears for past days only when a daily score was stored; recovery left is shown only for today. Missing readings, loading, and read errors are labeled separately.
...
- **Activity Calendar** is a full-width dashboard tile. It shows the current month and a short preview of the selected day's activities, notes, and plans. Earlier dates also show available health readings, while today's health stays in the Today summary. **Full day** shows every entry. Existing Calendar tiles expand once while other tiles keep their settings; add it from **Activity Overview** when you want it. The [Activity Calendar guide](/help#activity-calendar) explains its views, circles, summaries, and data scope.
...
- Event tags can be added from an event row or event details. The dashboard table's exact tag filter and tag editors list your saved tags regardless of the selected date range. A tag remains available for reuse after you remove it from every event. Selecting a tag filters events in the current date range; widen the range to see older matches. You can apply atomic add/remove tag changes in bulk to up to 250 selected events. Each event supports up to 10 tags of 32 characters; tags are visible on public event and comparison links.
...
- **Workout suggestion for today:** the Assistant checks today's recorded activity, recent weekday workout dates, and ready Training load snapshots. With the separate choices enabled, it also checks dated Timeline notes and today's planned workouts with their exact stored completion links. The answer lists the checked facts and any missing or incomplete data. An ended note is historical context, and a planned workout is not proof it was completed. A suggestion does not create or send a workout; a requested change still needs your review in Quantified Self.
- **Training:** Training metric catalog, preparation of selected snapshots, current values, Form, ramp, load, volume, intensity, current-versus-usual context, and missing or rebuilding states. If preparation takes longer than a short wait, try the same question again after a few seconds.
...
- Saved-route bounds, route geometry, route waypoints, full-resolution or unrequested sensor streams, original files, direct write tools, and dashboard settings remain unavailable even when precise activity locations are enabled. The only Assistant mutation paths are separately enabled, prepare-only Training, activity-tag, and Timeline-note proposals reviewed and confirmed in Quantified Self.
...
- **New chat** immediately replaces the stored conversation, removes its prior message content and pending proposals, and returns precise activity locations, Timeline notes, content changes, and all Training access choices to **off**.
...
Wahoo **Training** delivery is available to connected Pro members and is separate from activity uploads and route sending. If Wahoo needs plan access, use **Reconnect Wahoo** in Training sync and authorize the same account. Existing activity and route permissions remain requested. Reconnect alone does not enable plan sync: return to review the setup or retry the workout. Only time-based running/cycling workouts in the seven-day window can currently be delivered; cloud acceptance does not confirm ELEMNT/watch receipt. When Wahoo later returns the recorded activity with the exact QS-delivered Workout, Plan and token identifiers, Training can show **Completed · activity linked** without matching by title or date.
...
2. On the Quantified Self approval screen, select the missing permission. For Training, choose **Training plans and planned workouts** to read plans. Also select
   **Change Training plans and workouts** to propose schedule edits and/or **Change planned-workout sync** to propose
   ...
   Each change permission requires its matching read permission.
