...
- Power systems shows today plus sparse workout-date points from the latest 12 weeks. A value appears only when Sports-lib marks that component ready; partial, insufficient, poor-fit, unstable, and invalid evidence remains explicit instead of becoming zero. Diagnostics distinguish every usable power curve from the smaller set of activities that actually supplied the retained sustained and short-duration envelope anchors, and show fitting-method disagreement separately from anchor-removal sensitivity. This is capacity evidence—not TSS, FTP, fitness, fatigue, Readiness, or a workout prescription.
