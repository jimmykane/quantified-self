...
- Choose the workout's exact profile: Running, Trail Running, Treadmill, Cycling, Mountain Biking, Indoor Cycling, E-Biking, Hand Cycle, or **Pool swimming**. Pool-swim distance steps use metres and pace targets use your swim-pace unit (/100 m or /100 yd); the current recipe does not store pool length, so set your 25 m, 50 m, or yard pool on the device where needed. Suunto recommends Guides for the matching exercise profile, including Swimming. An authorized MCP client can also author the canonical Indoor or Virtual Running, Virtual Cycling, Velomobile, Enduro MTB, and Downhill Cycling profiles. Garmin keeps every authored running/cycling profile unchanged in QS but receives only its broad Running or Cycling family, so QS shows that difference for review before sending a subtype. The editor supports date-only scheduling, time or distance steps, fixed repeats, and one absolute heart-rate, power, or pace target per step. COROS can encode pool-swim steps without a target, but new COROS Send and plan sync actions remain **Coming soon** in the app. Garmin pool-swim delivery needs a separate pool-length and swim-step mapping and is not offered yet; Wahoo's public plan file covers running/cycling only. Unsupported combinations remain unavailable instead of being approximated.
