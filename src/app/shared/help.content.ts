...
- **Readiness today** uses the same current formula as the optional Dashboard Readiness Signals tile. It combines derived Form/ramp with a bounded 30-day sleep-only query; the browser does not load event or activity history. The latest non-nap night must be no more than 48 hours old, and HRV/minimum-heart-rate comparisons use up to 14 prior nights from the same provider. It re-evaluates automatically when time alone makes a future record eligible, expires the latest night, or removes baseline evidence. Score, status, confidence, calculation time, driver freshness, and missing signals remain separate. Failed load or sleep reads are identified separately from missing evidence. Sleep already loaded before a listener failure remains visible only while eligible, with available load-only context afterward. Its short training implication is context, not a workout instruction.
