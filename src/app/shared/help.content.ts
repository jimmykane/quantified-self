...
- Each tile’s three-dot menu uses the same layout and identifies its type: for example, **Edit KPI** and **Remove KPI**, or **Edit map** and **Remove map**. Removal only takes it off your dashboard; it does not delete the underlying data. You can remove the last tile; empty sections retain their add action. Shared dashboards remain read-only.
