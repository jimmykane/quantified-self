...
- Manual planning is available without a service connection. Garmin, Suunto and Wahoo workout sync is restricted to the private rollout. New COROS workout sync setup in the app is shown as **Coming soon**; existing enabled COROS sync keeps running. Connecting a service does not send anything automatically.
...
- **New COROS workout sync setup is coming soon in the app.** Existing enabled COROS sync continues in the background, and its delivery records, retry, copy status, and Stop sync remain available. The app does not currently offer new COROS enable, send, or resume actions. COROS plan sync sends workouts individually to the COROS training calendar rather than creating or claiming a native QS-owned COROS plan. [COROS documents a two-week watch window and one training plan synced to a watch at a time](https://support.coros.com/hc/en-us/articles/360048955151-Creating-and-Using-Training-Plans). COROS does not provide a planned-workout read/list operation, so remote checking and automatic missing-copy restoration are unavailable.
...
- Garmin, Suunto and Wahoo workout sync is restricted to the private rollout, not a public launch. New COROS workout sync setup in the app is shown as **Coming soon** while existing enabled COROS sync keeps running. New COROS enable/send/resume controls stay hidden; existing sync details, retry, and Stop sync remain available.
