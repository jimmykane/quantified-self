...
- **Power-system strain** appears in an individual workout’s Performance area when recorded power and its power curve support the three-dimensional model. It shows that workout’s sustained-power, finite-capacity, and maximum-power strain components with input and fit quality. It is not TSS, FTP, fitness, fatigue, or Readiness, and workouts from different activity types are never combined. Earlier protocol records ask for reprocessing; missing or ineligible evidence remains unavailable rather than zero.
