...
From **Route actions → Send to** on Route Details, Pro members can send the current saved route to eligible connected **Suunto**, **COROS**, **Garmin**, and **Wahoo** accounts. Only eligible destinations appear. Connect or reconnect a provider from **Connections** if it is missing; Garmin needs Course Import permission, and Wahoo may ask you to reconnect and allow route access. The route must belong to you and retain its original file. A **Sending route** indicator stays visible while the request runs, and repeat sends are blocked until it finishes. The result appears below the summary and in a notification. Sending an updated Suunto copy asks for confirmation because it creates another route in Suunto.
