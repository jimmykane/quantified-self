...
- Highlights show available **Sleep** and **Heart rate** context with every connected source kept separate. **HRV** appears as a source-specific 14-day trend only when that series has at least three observations; a lone sleep-derived HRV value already shown inside Sleep is not repeated as an empty card. Each source and semantic series builds its own 60-day personal range from distinct recorded nights. Until 14 nights exist, the status stays neutral and shows baseline progress. After that, a seven-day average with at least three recent nights is labeled and colored as within, outside, or far outside that personal range; unusually high and unusually low values are treated symmetrically. The chart keeps its normal metric color while the range band, latest-point marker, text label, and accessible description communicate status, so color is never the only signal. This is a personal trend, not a medical interpretation. Health never creates a cross-provider headline average or saves a preferred source.
