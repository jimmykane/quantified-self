...
Admins can use [Marketing campaigns](/admin/marketing) to write with a formatting toolbar and see the founder email layout update beside the editor at desktop or phone width, or as plain text. Save the draft before sending a test to an email address you choose. A test is marked in its subject, and its unsubscribe link cannot change anyone's preference. Editing the saved draft clears its earlier test result. Select plans and inclusive UTC signup dates, prepare the fixed recipient list, then start, pause, resume, or clone the campaign. A test must be accepted by the mail service before sending starts. The global UTC daily limit counts tests and retries. The delivery view distinguishes queued, SMTP-accepted, failed, and skipped recipients; SMTP acceptance does not prove inbox delivery or opens.
