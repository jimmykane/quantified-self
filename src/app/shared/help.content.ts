...
- Sleep access covers normalized session summaries and day/week/month aggregates. When you also grant **Activity and Training metrics**, a client can request a compact morning briefing in an explicit IANA timezone: only the latest completed non-nap sleep, a short same-provider duration comparison, and the current UTC-day Training readiness status. It excludes the provider identity, provider user/session IDs, provider payloads, raw sleep-stage intervals, score components, raw HRV, SpO2, respiration samples, locations, activities, body measurements, workout plans, and medical advice.
