...
- **New COROS plan sync setup is coming soon in the app.** Standalone Send remains available to connected Pro members. Existing enabled plan sync continues to follow its saved settings and plan status; delivery records, retry, copy status, and Stop sync remain available, while plan configuration and plan-workout resume are hidden. COROS plan delivery sends individual workouts to the COROS training calendar rather than creating or claiming a native QS-owned COROS plan. [COROS documents a two-week watch window and one training plan synced to a watch at a time](https://support.coros.com/hc/en-us/articles/360048955151-Creating-and-Using-Training-Plans). COROS does not provide a planned-workout read/list operation, so remote checking and automatic missing-copy restoration are unavailable.
...
- Workout delivery to Garmin, Suunto, and Wahoo is available to connected Pro members. COROS standalone Send is available, while new COROS plan sync setup in the app is coming soon. Every destination remains off until the user explicitly enables an available plan sync or sends a standalone workout. Provider-specific compatibility, permissions, scheduling windows, and connection checks still apply.
