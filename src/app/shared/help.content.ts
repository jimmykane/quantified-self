...
- Use **All sports** on desktop to switch between **All training** and Running, Cycling, Swimming, Rowing, Walking & Hiking, Nordic Skiing, Strength, or Paddling. On mobile, **All** and your swipeable sport shortcuts switch the common views in one tap; the compact **All sports** arrow button opens every sport with the current view marked, and **Manage sport shortcuts** moves shortcut editing into that same picker. Your last destination is saved to your account, not the URL. **Shortcuts** pins up to four sports for faster desktop and mobile switching; every sport remains in the complete picker. Until you save shortcuts, Training ranks them automatically from training duration and workouts in the latest 28 days, with saved sport benchmarks as fallback evidence. A fixed shortcut choice remains until you change it, and **Use automatic selection** restores automatic behavior. Shortcuts change navigation only and never hide a sport from totals or the complete picker.
...
- On **All training**, Training Mix gives every recorded registered family a compact current-versus-usual workout and duration summary, followed by the global intensity chart. A sport destination expands only that family's latest 28 days versus its normalized 84-day baseline. Context summaries keep materially different environments separate and show only recorded metrics appropriate to that profile: for example ascent/descent for vertical sports, gravity MTB jump count and longest jump, distance-weighted 500 m rowing pace plus cadence/stroke distance, and elapsed time without distance for Strength. Best build comparisons use the same context metrics. Missing TSS, zones, or profile metrics stay unavailable rather than being interpreted as zero.
