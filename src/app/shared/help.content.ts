...
- New dashboards place a full-width **Calendar** section just below Today. Select a date to preview its activities, notes, and plans beside the month or below it on a phone. Scroll the day preview to see more without changing the calendar's height. The preview shows the first two entries in each group and counts the rest. Earlier dates also show their available health readings; today's health stays in Today above. **Full day** opens a dedicated page with every entry and previous/next day controls.
...
- Existing Calendar tiles move into their own section without changing their saved settings. They expand to the full section width once; later manual resizes are kept. **Open calendar** in the section header opens the complete Calendar route. If you remove the tile, its section disappears. Restore it through **Add to dashboard → Calendar**; removing it keeps it out of suggestions until you add it manually.
...
- **Calendar** has its own full-width dashboard section below Today. It shows the current month and a short preview of the selected day's activities, notes, and plans. Earlier dates also show available health readings, while today's health stays in Today. **Full day** shows every entry. Existing Calendar tiles keep their settings; restore a removed one through **Add to dashboard → Calendar**. The [Activity Calendar guide](/help#activity-calendar) explains its views, circles, summaries, and data scope.
