...
- **Plan sync** manages automatic sync for that plan. Its **Workout sync status** list contains individual workouts, not plans or edits. Rows show the scheduled workout date and current sync status. Select anywhere on a row to open **Workout sync**, which names the parent plan (or Standalone), applies actions only to that workout, and offers **Back to plan sync**. **Edit workout** is a separate action at the top right beside the workout name, outside sync controls and attempt details; browsing sync details never opens the editor or changes consent.
