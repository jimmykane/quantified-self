...
- Totals use bars, scalar readings use lines or points, and categorical states use stepped series. Chart dates and tooltip times use the timezone offset recorded with each provider reading, so a local day is not relabeled as the preceding UTC day. Suunto Stress state keeps every category explicit and uses the app palette consistently across the line, points, axis labels, and tooltip marker: relaxing is green, active blue, passive gray, and stressful red. The state name and vertical position remain visible, so color is never the only signal. Every provider, connected account, aggregation, semantic variant, origin, recording method, and unit stays in a separate series. Provider-native or non-comparable readings are labeled and isolated from canonical readings.
...
- Manual planning is available without a provider connection. Garmin and Suunto workout delivery are restricted to the private rollout; connecting a service does not send anything automatically. COROS and Wahoo delivery are not enabled yet.
...
- Garmin and Suunto workout delivery are restricted to the private rollout, not a public launch. COROS and Wahoo delivery are not enabled yet. Send controls stay hidden for unavailable providers and accounts; existing delivery details and removal controls remain accessible if a provider is paused.
