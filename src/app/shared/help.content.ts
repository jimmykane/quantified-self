...
- Manual planning is available without a provider connection. Garmin workout delivery is available only to the private evaluation pilot; connecting a service does not send anything automatically. COROS, Wahoo and Suunto delivery are not enabled yet.
...
- Garmin workout delivery is restricted to the private evaluation pilot, not a public launch. COROS, Wahoo and Suunto delivery are not enabled yet. Send controls stay hidden for unavailable providers and accounts; existing delivery details and Stop sync remain accessible if a provider is paused.
...
- **Health charts on your dashboard.** Open **Add chart** in **Health** to browse the same metrics as the Health hub, plus **Sleep overview**. Sleep overview, Sleep duration, and Sleep score are separate choices. Search across Health categories, preview your readings, and add one tile per metric. No Health tiles are added automatically.
- Each Health tile has its own **Source and reading** selector, **1d**, **14d**, **30d**, **90d**, or **1y** range, and older/newer controls. Provider accounts and different kinds of readings stay separate. A selected reading with no data stays selected and explains what is missing. Changing one tile does not change another tile or your Health hub preferences. Detailed readings without long-range summaries offer **Show 30 days**.
...
- Use the compact action on the right of a section title: **Add KPI**, **Add chart**, or **Add map**. **Activity Overview** uses **Add tile** because it includes charts and a calendar. The action disappears when all presets in that section are already on your dashboard. Activity Overview keeps its action for custom charts; when all its presets are added, it opens custom chart properties directly.
- Search within the section, use **Load**, **Readiness**, or **Execution** to filter KPIs, and scroll the compact list. Each row identifies its type, data availability, and a small preview. The large preview identifies **Your data** or **Example data** explicitly. If a filter excludes the previewed item, select a matching row before adding.
...
- A **New** badge on a section’s add action means a newly released chart type is available, even if you do not have data for it yet. Opening that section’s list clears its badge across your devices; its new entries stay marked for that browsing session. Opening an existing tile’s settings does not clear badges. Existing library entries are not announced again, and shared dashboards do not show these badges.
