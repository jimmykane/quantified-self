...
- **Plan schedule** shows only the selected plan, even when paused or archived. Its start and end are marked; shaded dates fall outside the plan. Navigate its months, select a day, then use **Add workout** to prefill that day and plan. Empty days are simply unscheduled—not automatically rest days. On desktop you can edit a workout directly from its calendar label; on mobile, select its day to see the compact workout rows below the grid. The **Main Calendar** remains your separate overview of completed activities, standalone workouts, and the active plan.
