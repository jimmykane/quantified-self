...
- Choose the workout's exact profile: Running, Trail Running, Treadmill, Cycling, Mountain Biking, Indoor Cycling, E-Biking, Hand Cycle, **Pool swimming**, or **Open-water swimming**. Both swim profiles use metres for distance steps and your swim-pace unit (/100 m or /100 yd) for pace targets. For pool swimming, you can optionally select a physical pool length in metres or yards; this is separate from each step's distance. An empty pool length means unspecified, which some older devices may not support. Open-water swimming has no pool length. Suunto recommends Guides for the matching Swimming or Openwater swimming exercise profile, but does not receive your selected pool length, so check the watch's pool setting. An authorized MCP client can also author the canonical Indoor or Virtual Running, Virtual Cycling, Velomobile, Enduro MTB, and Downhill Cycling profiles through the existing v1 recipe tools, but the optional pool-length setting is not yet in those tools. Garmin keeps every authored running/cycling profile unchanged in QS but receives only its broad Running or Cycling family, so QS shows that difference for review before sending a subtype. The editor supports date-only scheduling, time or distance steps, fixed repeats, and one absolute heart-rate, power, or pace target per step. Garmin can send compatible target-free pool-swim workouts as lap swimming with your chosen pool length; an unspecified pool may not work on older devices. A **Found in connected app** cloud check does not confirm delivery to your watch. COROS can encode target-free pool-swim steps, but not open-water workouts; new COROS Send and plan sync actions remain **Coming soon** in the app. Garmin open-water and Wahoo swim delivery are unsupported. Unsupported combinations remain unavailable instead of being approximated.
...
2. On the Quantified Self approval screen, select the missing permission. For Training, choose **Training plans and planned workouts** to read plans. Also select
   ...
   **Individual activity details** and **Change events**. Each change permission requires its matching read permission.
