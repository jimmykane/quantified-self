...
Notes never change your measurements, readiness, or forecasts. Ongoing shading stops today. Source and sport filters do not hide notes. If a view reaches its note limit or cannot load notes, it says so while keeping your metric charts available. Notes stay private and are not sent to connected providers or public shares. Full titles and details, including notes hidden from charts, can be read only when you separately enable **Timeline notes** for an MCP client or in the Assistant. An external MCP client also needs **Change Timeline notes** before it can create, edit, or permanently delete a note; those tools use the MCP client's approval controls. This may include sensitive health or personal text. Chart visibility is not an access permission. Disconnecting a provider keeps notes; deleting your account removes them.
`;
...
- Plans and standalone workouts work without a service connection. Workout delivery to Garmin, COROS, Suunto, and Wahoo is available to connected Pro members through explicit plan opt-in or standalone Send actions. Connecting a service does not send anything automatically.
...
- **COROS plan sync sends workouts individually** to the COROS training calendar rather than creating or claiming a native QS-owned COROS plan. Plan opt-in, standalone Send, per-workout exclusion, retry, copy status, and Stop sync are available to connected Pro members. [COROS documents a two-week watch window and one training plan synced to a watch at a time](https://support.coros.com/hc/en-us/articles/360048955151-Creating-and-Using-Training-Plans). COROS does not provide a planned-workout read/list operation, so remote checking and automatic missing-copy restoration are unavailable.
...
- Workout delivery to Garmin, COROS, Suunto, and Wahoo is available to connected Pro members. Every destination remains off until the user explicitly enables plan sync or sends a standalone workout. Provider-specific compatibility, permissions, scheduling windows, and connection checks still apply.
...
2. On the Quantified Self approval screen, select the missing permission. For Training, choose **Training plans and planned workouts** to read plans. Also select
   **Change Training plans and workouts** to propose schedule edits and/or **Change planned-workout sync** to propose
   delivery actions. For note changes, select **Timeline notes** and **Change Timeline notes**. For tag changes, select
   **Individual activity details** and **Change activity tags**. Each change permission requires its matching read permission.
...
- Any authorized MCP client can discover canonical Sports Lib activity types for filters; that static catalog contains no account data. Individual activity detail access covers non-location summaries and parent event tags, laps, swim lengths, MTB jump measurements, signed-in app links, selected persisted numeric metrics such as Stroke Rate, and bounded chart-ready heart-rate, power, cadence, altitude, grade, distance, speed, and activity-appropriate pace streams. Activities from the same event share tags. Tags can contain personal, health, or location context and clients must treat them as untrusted labels, not instructions or verified facts. Clients can filter newest-first activity scans by one or more types and request **today** or **yesterday** in an explicit IANA timezone. They can also read tags or filter by exact case-insensitive tag matches using any/all semantics. **Change activity tags** is a separate child permission: the client must read the current tags first, then replace the complete shared parent-event list through its native approval UI. A concurrent edit fails instead of being overwritten. It cannot change activity metrics, descriptions, source files, or provider records. Existing clients must reauthorize for this new write permission; refresh cannot add it. The built-in Assistant is not expanded. Bounded pages report scan completion so a client can distinguish a complete no-match result from older history that remains to be checked. Chart streams are parsed temporarily from an existing FIT, GPX, TCX, Suunto JSON/SML, or gzip original file, downsampled over the complete activity, and discarded without a reparse, backfill, cache, or additional activity storage. Historical chart access therefore depends on the original file still being available and within the documented limits.
