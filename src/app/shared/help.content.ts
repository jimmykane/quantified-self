...
- Health opens on **Resting heart rate · 30d** when that metric is available. The explorer shows only metrics found anywhere in your imported history, regardless of the currently selected date window; Sleep appears when a normalized Sleep session exists. If availability cannot be checked safely, Health keeps the complete catalog visible rather than hiding valid data. Use **14d**, **30d**, **90d**, or **1y** and the older/newer controls. Your range is saved to your account without adding URL query parameters. The selected metric and older/newer position remain local to the open workspace and reset when you return later.
