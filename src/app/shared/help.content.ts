...
- Garmin workout delivery is restricted to the private production pilot, not a public launch. COROS, Wahoo and Suunto delivery are not enabled yet. Send controls stay hidden for unavailable providers and accounts; existing delivery details and Stop sync remain accessible if a provider is paused.
...
- Pausing or archiving a plan, or activating another plan, removes eligible future provider copies but retains preferences for reactivation. **Stop plan sync** ends sync for the plan and removes its eligible future copies. **Stop workout sync** affects only that workout; other workouts in the plan keep their settings. **Resume workout sync** removes that individual suppression. Past or provider-completed workouts remain unchanged.
