...
Use **Timeline notes** in the Dashboard, Health, Training, or Calendar header to add private context such as sickness, injury, vacation, travel, or stress. Notes are available to every signed-in account; no subscription or connected device is needed. **Add measurement** stays separate.
...
- Manual planning is available without a provider connection. Garmin, COROS, Suunto and Wahoo workout delivery are restricted to the private rollout; connecting a service does not send anything automatically.
- **Suunto sends individual SuuntoPlus Guides**, not a native Suunto plan. With sync enabled, QS sends today and the next six days in your saved delivery time zone. Later workouts show **Scheduled for later** and are sent automatically when their date approaches. Moving a sent upcoming workout beyond that window removes its old Guide until the new date approaches; past Guides stay unchanged.
...
- COROS plan sync sends upcoming workouts individually to the COROS training calendar; it does not create or claim a native QS-owned COROS plan. The COROS app manages watch synchronization. [COROS documents a two-week watch window and one training plan synced to a watch at a time](https://support.coros.com/hc/en-us/articles/360048955151-Creating-and-Using-Training-Plans), so **Sent** is not confirmation that a watch received the workout. COROS does not provide a planned-workout read/list operation, so remote checking and automatic missing-copy restoration are unavailable.
- Completed Suunto FIT files may carry identifiers for QS Guides used during a session. When one marker unambiguously belongs to the same connected account and scheduled workout, QS shows **Activity linked** and protects the provider copy from later automatic removal. This means the Guide was referenced by a recorded session; it does not prove that every prescribed interval or target was completed. Conflicting or ambiguous markers stay unlinked. The Guide ID and account evidence remain private, and deleting the activity or your QS account removes the local link and evidence.
- Completed Wahoo activities can link to a QS-delivered workout when Wahoo returns the exact Workout, Plan and app-supplied workout token under the same connected account. QS then shows **Activity linked** and protects that provider copy. This proves which delivered workout produced the recorded activity; it does not prove that every prescribed interval or target was completed. Missing, conflicting or ambiguous identifiers stay unlinked. The provider identifiers and account evidence remain private, and deleting the activity removes its local link and evidence.
- Choose the workout's exact profile: Running, Trail Running, Treadmill, Cycling, Mountain Biking, Indoor Cycling, E-Biking, or Hand Cycle. Suunto uses that choice to recommend the Guide for the matching exercise profile; generic Cycling is not guessed to mean Mountain Biking. Garmin supports the same choices, but its Training API receives only the broad Running or Cycling family, so QS shows that difference for review before sending a subtype. The editor supports date-only scheduling, time or distance steps, fixed repeats, and one absolute heart-rate, power, or pace target per step. Other services keep their own supported sport list, and unsupported combinations remain unavailable instead of being approximated.
...
- Where supported, active sync checks daily and restores workouts confirmed missing from the connected app. Provider limits can delay checking or restoration. **Stop sync** prevents restoration. Provider copies for past dates or completed workouts are kept.
...
- Garmin, COROS, Suunto and Wahoo workout delivery are restricted to the private rollout, not a public launch. Send controls stay hidden for unavailable providers and accounts; existing delivery details and removal controls remain accessible if a provider is paused.
- Wahoo initially supports time-based running and cycling workouts for today and the next six days in the saved time zone. Later dates wait automatically. Distance-based steps are not sent because Wahoo needs a total duration. The editor is unchanged. Bike computers use the first target in an interval; review any warnings before approving. Sent is cloud delivery, not a device receipt. Checks confirm the Plan, Workout and association; automatic restoration is unavailable.
- Garmin delivery needs **Training** permission and a current authorized connection. If permission or connection details are missing, reconnect Garmin in Connectivity and allow training workouts. Older connections may need this even when activity imports work. Garmin workout content and its calendar entry are separate: a delivery can need recovery after only one was accepted. If acceptance cannot be established, **Delivery uncertain** stays blocked to avoid duplicates; Retry may still require support rather than sending another copy. This does not change activity imports or mark a planned workout completed.
- When available, choose **Sync plan with Garmin** beside a plan's actions, then **Enable plan sync**. Availability and compatibility are checked automatically before that confirmation; there is no separate Preview step. New workouts and later edits sync automatically while the plan is active. For a standalone workout, choose **Send to Garmin**, then **Send workout** once; later edits update the same provider copy. Connecting a service alone never opts in. Provider delivery requires Pro; manual planning remains free.
- **Plan sync** manages automatic sync for that plan. The plan shows a quiet summary row with each provider's logo and a compact synced/total count. Choose the small **View** action for provider names and full status details. With more than one service, the dialog shows whether sync is enabled for each provider and one **Manage** action. Manage opens only that service's settings and individual workout statuses; **All services** returns to the provider overview. These navigation actions do not enable sync or send anything. The **Workout sync status** list contains individual workouts, not plans or edits. Rows show the scheduled workout date and current sync status. Select anywhere on a row to open **Workout sync**, which names the parent plan (or Standalone), applies actions only to that workout, and offers **Back to plan sync** in the same provider detail. **Edit workout** is a separate action at the top right beside the workout name, outside sync controls and attempt details; browsing sync details never opens the editor or changes consent.
- Plans and saved workouts show which services they have synced to. For example, **Garmin Connect · 2 of 3 workouts synced** counts workouts across the whole plan, not just the selected day. Waiting, skipped, paused and blocked workouts stay in the total with their status explained. Select the service summary for details. Older-account copies do not count as current sync, and **Status incomplete** means a full total is unavailable. The editor reports the saved workout, not unsaved changes. These summaries describe individual workout delivery, even when a service has no native plan support; they do not confirm that a watch has received the workout.
...
- The automatic check is read-only and can be cancelled. Saving settings starts background work, not instant delivery. **Synced** and **Up to date** mean the full delivery to the provider was confirmed, not delivery to a device. A provider copy can exist while its calendar entry or latest changes are still unconfirmed; this does not mean the workout is corrupt. **Next automatic check** shows when recovery can run. **Show attempt details** reveals compact timestamps and retry counts; **How sync works** opens lifecycle guidance. You can close the dialog while delivery continues.
- Pausing or archiving a plan, or activating another plan, removes eligible future provider copies but retains preferences for reactivation. **Stop plan sync** ends sync for the plan and removes its eligible future copies. For just one plan workout, open the provider's more-actions menu and choose **Exclude from plan sync**. The workout stays in your QS plan; only its upcoming provider copy is removed, if one exists. Other workouts keep syncing. **Resume workout sync** includes it again. Standalone workouts use **Stop workout sync**. **Past date · provider copy kept** explains an unrelated past delivery. When an exact activity is linked, the evidence provider shows **Completed · activity linked** and other confirmed provider copies show **Sent · workout completed**.
...
- Stopping workout sync keeps your provider account connected and your plans and workouts in Quantified Self. If you separately choose to disconnect a provider or delete your Quantified Self account, workouts already sent may remain in the provider's app. To request removal of upcoming synced workouts through Quantified Self, stop sync while the account is still connected. Disconnecting turns off workout sync for that connection; reconnecting does not turn it back on. An authentication failure keeps your sync settings for reconnecting the same account; connecting a different account requires you to enable sync again.
...
- **Add tiles anywhere.** Use **Add to dashboard +** at the top (the **+** button on phones), then choose a dashboard section. Empty sections stay hidden until you add their first tile and disappear when you remove their last one. A completely empty dashboard offers **Add tiles** or **Use starter dashboard**.
...
- Populated sections keep the compact action on the right of their title: **Add KPI**, **Add chart**, or **Add map**. **Activity Overview** uses **Add tile** because it includes charts and a calendar. The section action disappears when all its presets are already on your dashboard; the top Add action stays available. Activity Overview keeps its action for custom charts; when all its presets are added, it opens custom chart properties directly.
...
- The add action opens a wide dialog on desktop or a bottom sheet on mobile. Select a row to see its large preview, explanation, data scope, and destination section. Desktop previews the first available item and keeps the list beside it. Mobile opens the list first; **Back to KPIs**, **Back to charts**, **Back to maps**, or **Back to tiles** returns to your selected row in that section’s list.
...
- Today shows **Loading readiness…** until the initial load and sleep checks finish. If no eligible sleep is found, the score uses the available load signal. A failed sleep check is labelled separately from missing sleep; previously loaded sleep remains usable while eligible. Newly imported data can still update the score afterward.
...
- The main dashboard groups chart and map tiles by intent, **Training State**, **Health**, **Performance & Power**, **Activity Overview**, and **Routes & Maps**.
...
- **Training planning (optional):** turn on **Training plans** in **Examples & data access** to read current plans, standalone workouts, full instructions, exact stored completion links, step notes and existing service sync status. **Plan and workout changes** and **Planned-workout sync changes** are separate default-off choices and require Training plans access. Gemini can prepare one bounded proposal but cannot apply it. Review the app-owned summary, then choose **Apply changes** or **Dismiss**. Changing access starts a fresh chat; **New chat** resets all optional permissions and invalidates older proposals. Provider delivery remains Pro and connection/rollout gated; a delivery failure never removes an authored workout. Confirmed sync means provider-side workout delivery, not receipt on a watch.
...
- Saved-route bounds, route geometry, route waypoints, full-resolution or unrequested sensor streams, original files, arbitrary write tools, and dashboard settings remain unavailable even when precise activity locations are enabled. The only Assistant mutation path is the separately enabled Training proposal reviewed and confirmed in Quantified Self.
...
- **New chat** immediately replaces the stored conversation, removes its prior message content and pending Training proposal, and returns precise activity locations, Timeline notes and all Training access choices to **off**.
...
- Free permission-scoped MCP connections, including confirmed Training plan and workout changes
...
Private-rollout **Training** sync is separate from activity uploads and route sending. If Wahoo needs plan access, use **Reconnect Wahoo** in Training sync and authorize the same account. Existing activity and route permissions remain requested. Reconnect alone does not enable plan sync: return to review the setup or retry the workout. Only time-based running/cycling workouts in the seven-day window can currently be delivered; cloud acceptance does not confirm ELEMNT/watch receipt. When Wahoo later returns the recorded activity with the exact QS-delivered Workout, Plan and token identifiers, Training can show **Activity linked** without matching by title or date.
...
- **Timeline notes** is an independent read-only permission for full private titles and details, categories, actual dates and captured time zones, including notes hidden from charts. It is selected by default when requested; uncheck it before approving to withhold access. Existing clients must reauthorize. Queries use inclusive calendar windows of at most 366 days and return full text in bounded pages. Ongoing periods end today in their original time zone. This text may contain sensitive health or personal information. Revocation blocks future access but cannot erase copies already received. Notes permission never grants Training plan writes.

- An MCP client receives only the capabilities you approve after signing in. Every requested permission starts checked, including permissions added later; uncheck anything you do not want to grant. Data categories remain separate read permissions. Training changes require their explicit child permission and a second confirmation for each bounded proposal. Activity locations require activity details; saved-route locations require saved-route summaries. Removing activity details also removes its descriptions and location permissions; removing any parent removes its dependent children.
