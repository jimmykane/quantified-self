...
- Use **Sports shown** to personalize detail cards for Running, Cycling, Swimming, Rowing, Walking & Hiking, Nordic Skiing, Strength, and Paddling. Until you save a choice, Training selects families automatically from activities in the latest 28 days and saved sport benchmarks. On the initial load, sport cards stay hidden until that evidence is ready; when none qualifies, only the global Training sections remain. After a valid summary has loaded, automatic selection keeps those sport cards visible while a replacement snapshot refreshes. A saved choice remains fixed until you change it, and **Use automatic selection** restores automatic behavior.

## Sports and multisport activities

- Running keeps road, trail, treadmill, indoor, and virtual contexts separate where that changes the available summary. Cycling includes road, indoor, virtual, e-bike, standard mountain biking, Enduro MTB, and Downhill MTB. Swimming separates pool and open water. Rowing separates indoor and on-water sessions. Walking & Hiking includes Walking, Nordic Walking, Hiking, and Trekking. Nordic Skiing includes cross-country, Nordic, and roller skiing. Strength includes Strength Training, Weight Training, and Kettlebell. Paddling includes Canoeing, Kayaking, Paddling, and Stand Up Paddling. Nearby but unlisted sports such as CrossFit, ski touring, snowshoeing, surfing, and sailing remain Other rather than being guessed into a family.
...
- Hiding a sport removes only that family's **Best build vs now**, Training Mix, and capability-matched specialist cards. It does not filter the overall comparison, **What drove this**, or **Power systems**. Power systems discovers every exact canonical activity type with usable stored power curves independently; it never combines related types into an all-sports value. Global load and intensity charts can still include other eligible activities when their source data provides TSS or zones.
...
- Training Mix compares the latest 28 days with a normalized 84-day baseline for every registered Training family. Context summaries keep materially different environments separate and show only recorded metrics appropriate to that profile: for example ascent/descent for vertical sports, gravity MTB jump count and longest jump, distance-weighted 500 m rowing pace plus cadence/stroke distance, and elapsed time without distance for Strength. Best build comparisons use the same context metrics. Missing TSS, zones, or profile metrics stay unavailable rather than being interpreted as zero.
...
COROS activity upload and activity delivery are available to all eligible connected Pro users. The three route features above are currently shown only to approved route-pilot accounts while COROS production route-push permission is being verified.
