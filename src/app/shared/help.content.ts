...
- Sleep access covers normalized session summaries, day/week/month aggregates, bounded discovery of recorded safe aggregate vital types, and a one-call sleep trend that combines coverage with duration, score, stages, HRV, heart-rate, blood-oxygen, and respiration values for the requested period. Raw samples remain excluded, and the result cannot diagnose illness. When you also grant **Activity and Training metrics**, a client can request the same live UTC-day Readiness used by Dashboard Today: current Form/ramp plus the latest eligible sleep score, safe aggregate HRV and overnight-heart-rate values, same-provider baseline medians, ratios, evidence counts, and explicit missing or insufficient-baseline states. The IANA timezone supplies local-day context while Readiness remains UTC-day based. A separate compact morning briefing returns the latest completed non-nap sleep, a short same-provider duration comparison, current-versus-usual equivalent 28-day Training totals and Running/Cycling/Swimming mix, and the compact current readiness status. These projections exclude provider identity, provider user/session IDs, provider payloads, raw sleep-stage intervals, score components, raw HRV samples, SpO2 and respiration samples, locations, activities, body measurements, workout plans, and medical advice.
