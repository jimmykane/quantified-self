...
- **Plan sync** manages automatic sync for that plan. The plan shows one **Plan sync** entry with a line for each connected service; all of those lines belong to the same overview. In that overview, choose a provider tab to see only that service's settings and workout statuses. Changing tabs does not enable sync or send anything. The **Workout sync status** list contains individual workouts, not plans or edits. Rows show the scheduled workout date and current sync status. Select anywhere on a row to open **Workout sync**, which names the parent plan (or Standalone), applies actions only to that workout, and offers **Back to plan sync** in the same provider tab. **Edit workout** is a separate action at the top right beside the workout name, outside sync controls and attempt details; browsing sync details never opens the editor or changes consent.
