...
- Sleep access covers normalized session summaries, day/week/month aggregates, and a bounded discovery of which safe aggregate vital types have recorded values in a requested period. This lets a client find available nightly or averaged HRV, heart-rate, blood-oxygen, and respiration values before querying them; raw samples remain excluded. When you also grant **Activity and Training metrics**, a client can request a compact morning briefing in an explicit IANA timezone: the latest completed non-nap sleep, a short same-provider duration comparison, current-versus-usual equivalent 28-day Training totals and Running/Cycling/Swimming mix, and the current UTC-day Training readiness status. It excludes the provider identity, provider user/session IDs, provider payloads, raw sleep-stage intervals, score components, raw HRV, SpO2, respiration samples, locations, activities, body measurements, workout plans, and medical advice.
