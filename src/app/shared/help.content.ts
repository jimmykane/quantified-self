...
- An MCP client can read data only after you sign in and approve its requested permissions. **Activity, body measurements, and Training metrics**, **Individual activity details**, **Sleep summaries**, and **Saved routes and waypoints** are separate, optional read-only permissions.
- Metric access covers persisted numeric activity metrics, first-class body-measurement history, and ready Training-derived snapshots. Body-weight history is available for bounded ranges up to 366 days as identity-free day, week, or month values using median, average, minimum, maximum, or latest aggregation. It contains recorded measurements only and is not a medical or health assessment. It excludes exact source measurement timestamps, event/activity identity, names, provider/device metadata, and source provenance. When individual activity detail access is also granted, a client can request up to 25 explicitly selected canonical numeric Sports Lib metrics for one referenced activity. Precise latitude/longitude metrics are excluded, and Training event/activity IDs, names, labels, source fingerprints, and imported device/provider source keys are removed.
- Individual activity detail access covers bounded activity summaries with exact start/end latitude/longitude coordinates when available, nearby searches that match an activity's start or end, laps, swim lengths, MTB jumps, and signed-in app links containing stable account/event paths. Together with metric access, it also permits explicitly selected canonical numeric metrics for one activity. Jump records can also include exact latitude/longitude coordinates. Exact activity and jump coordinates can reveal your home, workplace, frequent trailhead, or other sensitive locations. Raw streams, activity names and notes, original files, device/provider provenance, precise-position metrics, nonnumeric stats, and unrequested stored stats are excluded.
...
4. Start a new chat, select the Quantified Self app, and ask about activity metrics, body-weight history or trends, activity details, sleep summaries, saved routes, activities that started or ended near a place, or routes that pass near a place.
