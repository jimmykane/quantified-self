...
- Manual planning is available without a service connection. Garmin, COROS, Suunto and Wahoo workout sync is restricted to the private rollout; connecting a service does not send anything automatically.
...
## Connected service workout sync

- Garmin, COROS, Suunto and Wahoo workout sync is restricted to the private rollout, not a public launch. Send controls stay hidden for unavailable services and accounts; existing sync details and removal controls remain accessible if a service is paused.
...
- The automatic check is read-only and can be cancelled. Saving settings starts background work, not instant delivery. **Synced** and **Up to date** mean the complete workout sync was confirmed in the connected app, not delivery to a device. A sent workout can exist while its calendar entry or latest changes are still unconfirmed; this does not mean the workout is corrupt. **Next automatic check** shows when recovery can run. **Show attempt details** reveals compact timestamps and retry counts; **How sync works** opens lifecycle guidance. You can close the dialog while sync continues.
- Pausing or archiving a plan, or activating another plan, removes eligible future copies but retains preferences for reactivation. **Stop plan sync** ends sync for the plan and removes its eligible future copies. For just one plan workout, open the service's more-actions menu and choose **Exclude from plan sync**. The workout stays in your QS plan; only its upcoming copy is removed, if one exists. Other workouts keep syncing. **Resume workout sync** includes it again. Standalone workouts use **Stop workout sync**. **Past workout · previously sent** explains an unrelated past delivery. When an exact activity is linked, its service shows **Completed · activity linked** and other confirmed copies show **Sent · workout completed**.
...
- **Training plans and planned workouts** is an independent read permission for names, dates, complete instructions, authored notes, exact stored completion links and existing service sync summaries. Text may contain sensitive personal information. **Training plan and workout changes** and **Training provider delivery changes** are separate child permissions; either also requires the read permission. Existing clients must explicitly reauthorize and refresh cannot add access. A client first previews at most 25 strict changes. Nothing changes until you confirm that exact preview: supported clients show the confirmation directly, while other modern clients give you a short-lived Quantified Self review link. Sign in to the same account, review the listed changes, and choose **Confirm and apply**; do not approve a link you did not request. Leaving the page applies nothing. Legacy clients remain read-only. Safe authored changes exclude permanent workout deletion, plan deletion and history restoration. Provider changes remain Pro, connection and rollout gated and return independent results.
