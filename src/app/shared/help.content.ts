...
- When available, choose **Sync with Garmin** beside a plan's actions, then **Enable sync**. Availability and compatibility are checked automatically before that confirmation; there is no separate Preview step. New workouts and later edits sync automatically while the plan is active. For a standalone workout, choose **Send to Garmin**, then **Send workout** once; later edits update the same provider copy. Connecting a service alone never opts in. Provider delivery requires Pro; manual planning remains free.
- The confirmation shows the delivery time zone, defaulting to your browser's IANA time zone. Use **Change** and **Use time zone** only if you need a different zone. Travel does not silently change workout dates. Plan workouts inherit their plan's zone; change it explicitly in the plan's **Sync settings**.
...
- **Delivery history** appears in Plans once delivery records exist. Compact rows show each workout's current status and latest confirmation/attempt time, not a card for every edit. A newer failed attempt replaces an older confirmation time in the summary; **Show attempt details** keeps both available. Open a row's **Delivery details** arrow for warnings and recovery, even after you delete a workout or its plan; this does not restore the deleted workout or start new sync. Loaded details stay up to date automatically. Authored edits remain in the separate, compact, scrollable **Revision history** with restore actions.
