...
- Hiding a sport removes only that sport's **Best build vs now**, Training Mix, and sport-specific performance cards. It does not filter the overall comparison or notable changes, which always combine eligible Running, Cycling/Mountain Biking, and Swimming activity legs. Load and intensity charts can still include other eligible activities when their source data provides TSS or zones.
...
- Recovery context uses the longest valid main overnight record from each provider per sleep date; naps are excluded. Average sleep appears with at least three recorded nights, while bedtime variation and overnight HRV need at least five qualifying nights. Missing nights and missing HRV are never counted as zero, and sleep stages, provider scores, SpO₂, and respiration are not blended into a synthetic readiness score.
