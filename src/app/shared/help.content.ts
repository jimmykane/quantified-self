...
- The confirmation shows the delivery time zone, defaulting to your browser's IANA time zone. Use **Change** and **Use time zone** only if you need a different zone. Travel does not silently change workout dates. Plan workouts inherit their plan's zone; change it explicitly in **Plan sync settings**.
- The automatic check is read-only and can be cancelled. Saving settings starts background work, not instant delivery. **Up to date** means the full delivery was confirmed. A provider copy can exist while its calendar entry or latest changes are still unconfirmed; this does not mean the workout is corrupt. **Next automatic check** shows when recovery can run. **Show attempt details** reveals compact timestamps and retry counts; **How sync works** opens lifecycle guidance. You can close the dialog while delivery continues.
