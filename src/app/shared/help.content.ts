...
- Choose the workout's exact profile: Running, Trail Running, Treadmill, Cycling, Mountain Biking, Indoor Cycling, E-Biking, Hand Cycle, **Pool swimming**, or **Open-water swimming**. Both swim profiles use metres for distance steps and your swim-pace unit (/100 m or /100 yd) for pace targets. Pool length is not stored in the recipe, so set your 25 m, 50 m, or yard pool on the device where needed; open-water swimming has no pool length. Suunto recommends Guides for the matching Swimming or Openwater swimming exercise profile. An authorized MCP client can also author the canonical Indoor or Virtual Running, Virtual Cycling, Velomobile, Enduro MTB, and Downhill Cycling profiles. Garmin keeps every authored running/cycling profile unchanged in QS but receives only its broad Running or Cycling family, so QS shows that difference for review before sending a subtype. The editor supports date-only scheduling, time or distance steps, fixed repeats, and one absolute heart-rate, power, or pace target per step. COROS can encode target-free pool-swim steps, but not open-water workouts; new COROS Send and plan sync actions remain **Coming soon** in the app. Garmin swim delivery needs separate swim mapping and is not offered yet; Wahoo's public plan file covers running/cycling only. Unsupported combinations remain unavailable instead of being approximated.
