...
Each category has its own icon. Choose a **Color** in the editor to recognize a note in lists, on charts, and in Calendar, then select **Save**. **Default** restores the normal appearance. Chart markers show the note's title, shortened to fit when needed; open the note to read it in full. Grouped notes show a count. Overlapping notes with different colors share a neutral marker; opening it shows each note individually. Use the calendar buttons to pick dates, or type them in your local date format.
