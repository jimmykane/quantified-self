...
- **Workout VO₂ max is separate evidence.** Health reads each activity so Running, Cycling, and other disciplines keep their meaning, collapses consecutive unchanged estimates within the same source and discipline, and never merges them with provider Health or manual VO₂ max.
...
- Pausing or archiving a plan, or activating another plan, removes eligible future provider copies but retains preferences for reactivation. **Stop plan sync** ends sync for the plan and removes its eligible future copies. For just one plan workout, open the provider's more-actions menu and choose **Exclude from plan sync**. The workout stays in your QS plan; only its upcoming provider copy is removed, if one exists. Other workouts keep syncing. **Resume workout sync** includes it again. Standalone workouts use **Stop workout sync**. Past or provider-completed workouts remain unchanged.
