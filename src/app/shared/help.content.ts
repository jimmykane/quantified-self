...
- Plans and standalone workouts work without a service connection. Provider workout sync is restricted to the private rollout. New COROS plan sync setup in the app is shown as **Coming soon**; existing enabled COROS plan sync still follows its saved settings and plan status, while standalone **Send to COROS** remains available to eligible pilot accounts. Connecting a service does not send anything automatically.
...
- **New COROS plan sync setup is coming soon in the app.** Existing enabled COROS plan sync still follows its saved settings and plan status, and its delivery records, retry, copy status, and Stop sync remain available. The app does not offer COROS plan configuration or plan-workout resume actions while this gate is active. Standalone **Send to COROS** remains available to eligible pilot accounts. COROS plan sync sends workouts individually to the COROS training calendar rather than creating or claiming a native QS-owned COROS plan. [COROS documents a two-week watch window and one training plan synced to a watch at a time](https://support.coros.com/hc/en-us/articles/360048955151-Creating-and-Using-Training-Plans). COROS does not provide a planned-workout read/list operation, so remote checking and automatic missing-copy restoration are unavailable.
...
- Choose the workout's exact profile: Running, Trail Running, Treadmill, Cycling, Mountain Biking, Indoor Cycling, E-Biking, or Hand Cycle. Suunto uses that choice to recommend the Guide for the matching exercise profile; generic Cycling is not guessed to mean Mountain Biking. An authorized MCP client can also author the canonical Indoor or Virtual Running, Virtual Cycling, Velomobile, Enduro MTB, and Downhill Cycling profiles. Garmin keeps every authored profile unchanged in QS but receives only its broad Running or Cycling family, so QS shows that difference for review before sending a subtype. The editor supports date-only scheduling, time or distance steps, fixed repeats, and one absolute heart-rate, power, or pace target per step. Other services keep their own supported sport list, and unsupported combinations remain unavailable instead of being approximated.
...
- With **Training plans and planned workouts** access, a compatible MCP client can list plans, read complete workout instructions, query upcoming workouts in date order, inspect exact stored completion links, assess local provider compatibility, and read existing sanitized sync summaries. The MCP update must be released, discovered by your client, and explicitly authorized before these tools appear.
...
## Send workouts to connected providers (private beta)

- Provider workout sync is restricted to the private rollout, not a public launch. New COROS plan sync setup in the app is shown as **Coming soon** while existing enabled COROS plan sync still follows its saved settings and plan status. COROS plan configuration and plan-workout resume controls stay hidden; existing plan sync details, retry, and Stop sync remain available. Standalone **Send to COROS** remains available to eligible pilot accounts.
...
`,
  ...
  },
  {
    ...
    summary: 'Create plans and standalone workouts, use approval-gated MCP planning, and manage private-beta provider delivery.',
