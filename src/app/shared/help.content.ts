...
Each category has its own icon. Choose a **Color** in the editor to recognize a note in lists, on charts, and in Calendar, then select **Save**. **Default** uses the same neutral gray in lists, charts, and Calendar. Chart markers show the note's title, shortened to fit when needed; open the note to read it in full. Date ranges have start and end arrows with a subtle band between them. An open circle means the period continues beyond the view or is ongoing; ongoing shading stops at today. Single days, and ranges within one weekly chart bucket, use one marker. Grouped notes show a count. Overlapping notes with different colors share a neutral marker; opening it shows each note individually. Use the calendar buttons to pick dates, or type them in your local date format.

The manager lists all notes, including future plans, newest start date first. You can edit, delete with confirmation, or browse older pages. **Show on charts and calendar** is the global switch remembered across Health, Sleep, Training, and Calendar. Each note also has **Show this note on charts and calendar**, enabled by default; change it in the editor and select **Save**. A note appears only when both switches are on. Turning the global switch back on preserves your individual choices. Hidden notes remain editable in the manager with a **Hidden** label; hiding does not delete them. Tap a note's title or start/end marker to open it, or a grouped marker to see the overlapping notes. Hover or tap inside the shaded band to inspect your measurements as usual. The manager is also the keyboard-accessible way to browse them. Weekly charts show which notes overlap a week while retaining their actual dates.
...
- Search within the section, use **Load**, **Readiness**, or **Execution** to filter KPIs, and scroll the compact list. Each row identifies its type and includes a small preview with a **Your data** or **Example data** label. If a filter excludes the previewed item, select a matching row before adding.
...
- **Your data** previews use available personal data. **Example data** previews use synthetic examples when data is missing or unavailable, including while personal data is loading. Browsing never adds anything or starts a metric rebuild.
- The settings action above the preview matches your selection: **KPI settings**, **Chart settings**, **Map settings**, or **Calendar settings**. Properties appear beside the live preview on desktop and above it on mobile. The matching **Back to KPI**, **Back to chart**, **Back to map**, or **Back to calendar** action returns to the preview and preserves unsaved properties. Nothing is saved until **Add to dashboard** or **Save changes**; a successful save closes the picker and reveals the saved item. Leaving changed settings asks whether to discard them.
...
- Each tile’s three-dot menu uses the same layout and identifies its type: for example, **Edit KPI** and **Remove KPI**, or **Edit map** and **Remove map**. Where resizing is available, choose **Columns** or **Rows**, then a size; the current choice is marked. You can navigate these menus with the arrow keys. A spinner appears while a layout change is saving. Closing an editor returns keyboard focus to that tile’s menu button. Removal only takes it off your dashboard; it does not delete the underlying data. You can remove the last tile; empty sections retain their add action. Shared dashboards remain read-only.
