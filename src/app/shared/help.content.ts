...
- Manual planning is available without a provider connection. Garmin workout delivery is available only to the private production pilot; connecting a service does not send anything automatically. COROS, Wahoo and Suunto delivery are not enabled yet.
...
- Any authorized MCP client can discover canonical Sports Lib activity types for filters; that static catalog contains no account data. Individual activity detail access covers non-location summaries and parent event tags, laps, swim lengths, MTB jump measurements, signed-in app links, selected persisted numeric metrics such as Stroke Rate, and bounded chart-ready heart-rate, power, cadence, altitude, grade, distance, speed, and activity-appropriate pace streams. Activities from the same event share tags. Clients can filter newest-first activity scans by one or more types and request **today** or **yesterday** in an explicit IANA timezone. They can also read tags or filter by exact case-insensitive tag matches using any/all semantics. This uses the existing permission, so existing clients do not reauthorize, but a cached client may need to refresh its tool catalog. The built-in Assistant is not expanded. Bounded pages report scan completion so a client can distinguish a complete no-match result from older history that remains to be checked. Chart streams are parsed temporarily from an existing FIT, GPX, TCX, Suunto JSON/SML, or gzip original file, downsampled over the complete activity, and discarded without a reparse, backfill, cache, or additional activity storage. Historical chart access therefore depends on the original file still being available and within the documented limits.
