...
- A preview only checks delivery; it does not send workouts or change sync settings, and you can cancel it. Saving settings starts background work, not instant delivery. **Up to date** means the full delivery was confirmed. A provider copy can exist while its calendar entry or latest changes are still unconfirmed; this does not mean the workout is corrupt. **Next automatic check** shows when recovery can run. Expand **Attempt details** for timestamps and retry counts, or **How sync works** for lifecycle guidance. You can close the dialog while delivery continues.
