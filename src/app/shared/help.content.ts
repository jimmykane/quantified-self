...
- The activity table beneath the duration bars lists normal activities in the selected Week, Month, or Year. It follows Calendar navigation exactly; use its search, tag filter, sorting, pagination, and row checkboxes, or use an activity's action menu to share, reprocess, download, or delete it without leaving your current Calendar period. The tag filter lists tags from your full activity history, while matching table rows stay within the selected period. Month tables exclude adjacent dates shown only to complete the grid.
...
- Event tags can be added from an event row or event details. The dashboard table's exact tag filter lists tags from your full activity history, regardless of the selected date range. Selecting a tag filters the events in the current date range; widen the range to see older matches. You can apply atomic add/remove tag changes in bulk to up to 250 selected events. Each event supports up to 10 tags of 32 characters; tags are visible on public event and comparison links.
