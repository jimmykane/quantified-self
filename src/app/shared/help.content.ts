...
- Select a plan or **Standalone**, then use **Add workout** in that view. The editor opens on its own; **Save workout** keeps you in the workout's chosen destination, and **Cancel** returns without saving. When opened from Calendar, Cancel returns to the same plan or Standalone view; dates within the plan stay selected. Save and Cancel stay available while scrolling on mobile.
