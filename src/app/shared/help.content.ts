...
- Detailed sample streams load for 1d, 14-day, and 30-day windows. Longer windows use stored summary observations. If a metric exists only as samples, Health asks you to choose a shorter range instead of showing an empty or misleading aggregate. The one-day view shows available intra-day samples and daily summaries; it does not imply that every metric is continuous. Provider support and delivery cadence determine whether a metric is continuous, intermittent, or summary-only.
...
- New dashboards start with the Activity Calendar tile. The optional Dashboard **Today** header greets the dashboard owner according to browser-local morning, afternoon, or evening time, using the first part of their display name when available and generic copy otherwise; the greeting stays hidden on shared dashboards. Today then begins with the same TSS-only **Training state** shown in Training and shows current **Readiness** with its score, confidence, available-signal count, Load, Sleep, HRV, and Overnight HR alongside **Open Training** and **Open Health**. Select its calendar icon to open a mini calendar for the current month, use its previous and next controls to browse months, then select an activity day for details. Use **Show Today summary** in Dashboard manager to show or hide it independently from chart and map tiles.
