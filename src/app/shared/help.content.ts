...
- The chart option **Include all recorded metrics** makes other chartable streams, such as Temperature, available in **Visible charts**. It adds choices but does not show them automatically. Benchmark comparisons always make all recorded chartable metrics available so every source can be inspected.
- **Visible charts** groups sport recommendations before other available metrics. Showing or hiding a chart creates a custom override for that event and selected-sport combination.
