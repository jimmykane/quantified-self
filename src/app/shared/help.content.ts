...
- **Weight from workouts is fallback profile context, not a weigh-in.** Health reads one value from each parent event so a multisport file does not repeat the same profile Weight for every child activity. It appears only for sources without a real Health Weight measurement in the active provider-filtered window. A future manual measurement suppresses workout fallback across the view.
