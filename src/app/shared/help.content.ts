...
- **Recovery context** groups an active **Recovery left** estimate with expandable **Sleep history** inside Readiness today. The countdown remains visible while sleep details are collapsed. Sleep history places recorded overnight sleep beside training without changing the Training state or claiming that sleep caused a performance change. It compares the current 28 days with the preceding 84 days. Every ready **Best build vs now** card separately keeps sleep where it directly compares the exact current and saved benchmark ranges, with full metrics and source notes under **Details**.
...
- When a recent activity supplies a still-active device recovery estimate, Training shows it as **Recovery left**. It updates each minute, is omitted when missing or elapsed, is not a readiness score, and does not change the Training state.
