...
- Highlights show available **Sleep** and **Heart rate** context with every connected source kept separate. **HRV** appears as a source-specific 14-day trend only when a nightly recovery series has at least three observations; a lone sleep-derived HRV value already shown inside Sleep is not repeated as an empty card. Activity-interval and Health Snapshot HRV remain available in the explorer but are not interpreted as nightly recovery. Health prefers a provider's dedicated overnight HRV over a generic Sleep-session average for the same account. Each displayed provider/account series builds its own 60-day personal range from distinct recorded nights. Until 14 nights exist, the status stays neutral and shows baseline progress. After that, a seven-day average with at least three recent nights is labeled and colored as within, outside, or far outside that personal range; unusually high and unusually low values are treated symmetrically. The HRV trend line, points, latest-point marker, status label, and accessible description all carry the current personal-range status, so the generic HRV purple does not compete with the status and color is never the only signal. This is a personal trend, not a medical interpretation. Health never creates a cross-provider headline average or saves a preferred source.
