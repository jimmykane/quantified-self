...
export const processWorkoutTask = onTaskDispatched({
    ...
    console.log(`[TaskWorker] Starting task for ${serviceName} item: ${queueItemId}`);

    const queueRef = admin.firestore().collection(`${serviceName}Queue`).doc(queueItemId);
    const queueDoc = await queueRef.get();

    if (!queueDoc.exists) {
        console.error(`[TaskWorker] Queue item ${queueItemId} not found in ${serviceName}Queue`);
        return;
    }
    ...
    try {
        ...
        await parseWorkoutQueueItemForServiceName(serviceName, queueItem as any);
