...
export async function processRouteDeliverySyncQueueItem(
    ...
    try {
        ...
        if (providerSendInProgress && isProviderOperationError(error) && error.disposition === 'auth_required') {
            logRouteProviderFailureDecision(queueItem, error, 'skip');
            await safelyWriteDeliveryMetadata(() => setSkippedDeliveryMetadata(queueItem, 'destination_not_connected', error.message));
            return updateToProcessed(queueItem, bulkWriter, {
                skippedReason: 'destination_not_connected',
                resultStatus: 'skipped',
            });
        ...
        if (providerSendInProgress && isProviderOperationError(error) && error.disposition === 'permission_required') {
            logRouteProviderFailureDecision(queueItem, error, 'skip');
