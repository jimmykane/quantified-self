...
const ROUTE_DELIVERY_SYNC_RECONCILIATION_PAGE_SIZE = 100;
...
async function deleteRouteDeliverySyncCandidateBeforeDispatch(
...
async function shouldDispatchRouteDeliverySyncCandidate(
    doc: admin.firestore.QueryDocumentSnapshot,
    userID: string | null,
): Promise<boolean> {
    if (!userID) {
        await deleteRouteDeliverySyncCandidateBeforeDispatch(doc, 'missing userID');
        return false;
    }
...
export async function reconcileRouteDeliverySyncQueueDispatches(nowMs = Date.now()): Promise<{
    inspected: number;
    dispatched: number;
    skippedRecent: number;
}> {
    const cloudTaskQueueId = config.cloudtasks.routeDeliverySyncQueue;
    const pendingCloudTasks = await getCloudTaskQueueDepthForQueue(cloudTaskQueueId, true);
    if (pendingCloudTasks >= MAX_PENDING_TASKS) {
        logger.info(`[RouteDeliverySyncDispatcher] Queue busy (${pendingCloudTasks} pending tasks), skipping dispatch reconciliation.`);
        return {
            inspected: 0,
            dispatched: 0,
            skippedRecent: 0,
    ...
    const scanLimit = MAX_ROUTE_DELIVERY_SYNC_QUEUE_SCAN;
    const pageSize = Math.min(ROUTE_DELIVERY_SYNC_RECONCILIATION_PAGE_SIZE, MAX_ROUTE_DELIVERY_SYNC_QUEUE_SCAN, MAX_PENDING_TASKS);
    ...
    while (scannedDocs.length < scanLimit) {
        ...
        let query = admin.firestore()
            .collection(ROUTE_DELIVERY_SYNC_QUEUE_COLLECTION_NAME)
    ...
    for (const candidate of candidates) {
        ...
        try {
            ...
            const wasTaskEnqueued = await enqueueRouteDeliverySyncTask(candidate.doc.id, candidate.dateCreated);
            if (!wasTaskEnqueued) {
                logger.info(`[RouteDeliverySyncDispatcher] Task not enqueued for ${candidate.doc.id}; leaving dispatch marker unchanged.`);
                continue;
            }
            if (!candidate.userID) {
                continue;
...
export const dispatchRouteDeliverySyncQueue = functions.region('europe-west2').runWith({
    timeoutSeconds: 300,
    memory: '256MB',
    maxInstances: 1,
