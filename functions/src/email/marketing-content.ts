...
export function renderMarketingContent(draft: MarketingCampaignDraft): { bodyHtml: string; bodyText: string; ctaHtml: string; ctaText: string } {
  ...
  const ctaHtml = draft.cta
    ? `<p style="margin:0 0 28px;"><a href="${escapeHtml(draft.cta.url)}" style="display:inline-block;background:#174ea6;color:#ffffff;font-family:Arial,Helvetica,sans-serif;font-size:15px;font-weight:700;text-decoration:none;padding:12px 20px;border-radius:4px;">${escapeHtml(draft.cta.label)}</a></p>`
    : '';
