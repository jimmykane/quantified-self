...
export const refreshCOROSAPIRefreshTokens = functions.region('europe-west2').runWith({timeoutSeconds: 180}).pubsub.schedule('every 2 hours').onRun(async (context) => {
  ...
  const querySnapshot = await admin.firestore().collectionGroup('tokens').where('serviceName', '==', SERVICE_NAME).where("dateRefreshed", "<=", (new Date()).getTime() - (0 * 24 * 60 * 60 * 1000)).limit(50).get();
