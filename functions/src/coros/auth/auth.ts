...
function getRefreshTokenPath(): string {
  return `/oauth2/refresh-token?client_id=${config.corosapi.client_id}&client_secret=${config.corosapi.client_secret}`;
}
/**
...
export function COROSAPIAuth(refresh = false): AuthorizationCode {
  ...
  return new AuthorizationCode({
    ...
    auth: {
      ...
      tokenPath: refresh ? getRefreshTokenPath() : ACCESS_TOKEN_PATH,
    },
    options: {
