...
describe('Training authority extraction compatibility', () => {
  ...
  it.each(['retained', 'pinned', 'ambiguous', 'untrusted', 'numeric', 'wrong-id', 'malformed-pin'])('resolves Suunto authority: %s', async scenario => {
    const ref = { collection: () => ref, doc: () => ref, limit: () => ref };
    ...
    const values = [
      { data: () => ({ connectionState: 'connected', connectionStateGeneration: 'connection', ...(scenario === 'pinned' ? { providerUserId: 'account' } : {}), ...(scenario === 'malformed-pin' ? { providerUserId: 123 } : {}) }) },
    ...
    if (['retained', 'pinned'].includes(scenario)) {
