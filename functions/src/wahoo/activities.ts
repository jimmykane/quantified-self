...
async function correctCompletedWahooActivityType(params: {
  ...
  if (!workoutId) {
    ...
    }, params.expectedWorkoutType.id);
  }

  try {
    await withWahooWorkoutWriteToken(params.userID, async (accessToken) => {
      const form = new URLSearchParams();
      form.set('workout[workout_type_id]', `${params.expectedWorkoutType?.id}`);
      form.set('workout[name]', `${params.expectedWorkoutType?.name}`);
    ...
    logger.info('Corrected Wahoo workout activity type after FIT processing.', {
      ...
      workoutId,
      workoutTypeId: params.expectedWorkoutType.id,
      workoutTypeName: params.expectedWorkoutType.name,
    });
  } catch (error) {
    logWahooActivityUploadRequestError(error, 'status');
    return toWahooActivityTypeCorrectionError(error, params.uploadId, params.expectedWorkoutType.id);
  }
}
