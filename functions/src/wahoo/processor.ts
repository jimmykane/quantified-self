...
import { deferQueueItemForPendingDisconnect, increaseRetryCountForQueueItem, markQueueItemSkipped, QueueResult, updateToProcessed } from '../queue-utils';
...
export async function processWahooWorkoutQueueItem(
  ...
  if (!tokenSnapshot.exists || tokenSnapshot.data()?.serviceName !== ServiceNames.WahooAPI) {
    return markQueueItemSkipped(queueItem, undefined, 'provider_not_connected');
  }
  ...
  try {
    ...
    return increaseRetryCountForQueueItem(queueItem, getWahooRetryError(error));
  }
}
