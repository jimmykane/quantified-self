...
describe('Assistant MCP session', () => {
  ...
  it('exposes content reads and prepare-only tools after per-chat consent', async () => {
    ...
    try {
      ...
      expect(session.tools.map(tool => tool.name)).not.toContain('update_activity_tags' as never);
      ...
      expect(capturedAuth).toMatchObject({
        ...
        scopes: expect.arrayContaining([MCP_OAUTH_SCOPES.ActivityTagsWrite, MCP_OAUTH_SCOPES.TimelineNotesWrite]),
      });
      ...
      expect(prepared.structuredContent).toMatchObject({
        kind: 'update_activity_tags', requiresConfirmation: true,
      });
