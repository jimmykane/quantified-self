...
import { getSuuntoActivityUploadStatus, uploadActivityFileToSuunto } from '../suunto/activities';
...
async function persistPendingDestinationUpload(
    ...
    if (!queueItem.ref || !uploadResult.uploadId) {
        throw new Error('Pending destination upload is missing queue persistence data.');
    }
    ...
    if (queueItem.destinationServiceName === ServiceNames.SuuntoApp && !destinationProviderUserID) {
        throw new Error('Suunto pending upload is missing its provider user identity.');
    }
    ...
    const updateResult = await updateQueueItemIfUserActive({
        ...
        phase: queueItem.destinationServiceName === ServiceNames.WahooAPI
            ...
            : 'before_activity_sync_suunto_pending_upload_persist',
        updateData: {
            destinationUploadID,
        ...
        actionDescription: `${queueItem.destinationServiceName} pending upload state persistence`,
    });
