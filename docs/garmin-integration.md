...
Garmin activity history remains a separate user-selected flow. Its picker defaults to the full latest rolling five-year window through today because every accepted activity-history request starts Garmin's 30-day cooldown; users can explicitly narrow the range before submitting. The callable still divides that selection into requests of at most 89 days, respects a stricter provider-reported minimum, and does not treat request acceptance as proof that callbacks or activities arrived.
