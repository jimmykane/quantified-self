...
`backfillGarminAPIHealth` is the canonical user-facing Garmin history callable. It requests the existing Sleep history and creates one durable `garmin_health_backfill` cursor spanning all ten supported Health families for every eligible connected Pro user while Garmin Health is enabled. The UI checks the server-owned operational switch before presenting the action and reports the scope returned by the callable. If the emergency switch is disabled, Sleep-only history remains available. The legacy `backfillGarminAPISleep` export temporarily aliases the same handler for already-open or cached clients; #625 tracks its removal after the compatibility window.
