...
Wahoo is a Pro-only activity integration. Quantified Self receives completed workout summaries through Wahoo webhooks, can request a user-selected range of workout history, can send retained Wahoo FIT activities to Suunto, and can deliver FIT activities plus user-selected FIT courses/routes to Wahoo. It does not send plans, sleep, or other non-activity data between providers.
...
- Direct, user-selected FIT course/route delivery from Wahoo Services. The callable parses the FIT course/route to provide Wahoo's required metadata, looks up Quantified Self's deterministic external ID, then creates or updates the Wahoo route. It does not create or retain a Quantified Self route.
...
9. Direct FIT course/route delivery validates a 20 MB base64-bounded request, parses the course/route server-side, derives the required distance, ascent, start coordinate, activity-family, title, and deterministic external ID, then calls `GET /v1/routes?external_id=...` followed by `POST /v1/routes` or `PUT /v1/routes/:id`. It requires `routes_read` and `routes_write`; no browser token, FIT payload, or route document is persisted. Wahoo Cloud API routes sync to the Wahoo App and directly to an ELEMNT bike computer, not the ELEMNT App.
10. Queue documents expire through the shared queue TTL policy. Disconnect and account deletion write cleanup tombstones before recursively removing matching operational documents.
...
- Outbound requests are URL-encoded, carry the FIT as Wahoo's documented base64 data value, and never log the source file, bearer token, or upload form body. Wahoo's asynchronous activity-upload token—not the FIT payload—is persisted on an activity-sync queue item. Direct course/route delivery is synchronous and retains neither the payload nor a Quantified Self route document.
