...
- Wahoo identity transfer and FIT publication are serialized through leases on the server-only identity mapping. The worker holds its lease from first event write through user-readable FIT promotion; a transfer is rejected while any lease remains active. If a guarded write detects an ownership change, recursive cleanup removes the partial event and linked activities before retry/skip handling.
