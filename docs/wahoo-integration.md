...
2. The backend exchanges the code, reads the stable Wahoo user ID, and stores rotating credentials in `wahooAPIAccessTokens/{firebaseUid}/tokens/{wahooUserId}`. Each OAuth write assigns a server-side credential generation. The shared OAuth lifecycle queries the composite token index and removes stale tokens for the same external account from other Quantified Self users, matching the other provider adapters.
...
5. History imports use the same queue path. A per-user lease prevents overlapping history requests, pages stop once the selected start date is reached, and Wahoo rate-limit reset metadata is returned on HTTP 429.
...
8. For Garmin, COROS, and Suunto activity-sync routes whose destination is Wahoo, the shared worker downloads the already retained original FIT, creates the documented URL-encoded `POST /v1/workout_file_uploads` payload (`file`, optional `filename`, and optional `time_zone`), persists Wahoo's upload token before retrying, and polls `GET /v1/workout_file_uploads/:token` until it reaches `complete`, `duplicate`, or an error. The worker does not post the FIT again after an asynchronous upload has started. A generic opaque Wahoo refresh `400` first applies a durable per-account 5- then 15-minute pause. Three such failures within 24 hours mark the connection **Reconnect required**. New activity and saved-route delivery is parked rather than skipped. Reconnection restores only routes that had been enabled before the reconnect state, then releases that parked work. If a multi-collection release is partial, a server-only Wahoo metadata marker is retried by the existing 30-minute repair scheduler until the idempotent release completes.
9. Direct FIT activity delivery uses the same Wahoo upload helper and bounded status callable, but no event or source file is persisted by Quantified Self.
...
- Generic Wahoo refresh `400` responses are treated as an opaque provider anomaly, not as an immediate token deletion: a server-owned account cooldown protects the shared provider allowance, then a short repeated-failure threshold requires user reconnection. The recovery marker has no credentials or provider response body. A successful OAuth callback restores only previously enabled Wahoo routes, then releases only Wahoo activity and route queue items parked for this state. A partial release is persisted as a separate server-only repair marker and retried by the scheduled lifecycle worker.
