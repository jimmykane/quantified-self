...
- Wahoo FIT persistence uses a transaction guard over the server-only token root, exact token, and claimed queue item. Each guarded document write conflicts with a pending disconnect, token deletion, or queue supersession, and the original FIT remains in backend-only staging until the final guard succeeds. If a guard rejects after partial persistence, recursive cleanup removes only document roots created by that attempt, preserving previously imported revisions that share the deterministic event ID.
