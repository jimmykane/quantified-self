...
3. Every provider request obtains credentials through the shared refresh coordinator. It transactionally claims a short lease for an expired credential, so exactly one activity, route, history, or scheduled worker asks Wahoo to rotate it. The owner can persist only while its lease and credential generation still match; contenders use the winner's replacement or retry after the lease, and an expired lease is reclaimable after a worker crash. This prevents a late refresh from restoring an invalidated Wahoo token after reconnect or disconnect.
