...
If preparation fails, the campaign returns to draft. A hard timeout may prevent that cleanup; after an 11-minute lease expires, admins can retry preparation. The retry replaces any partial recipient snapshot before marking the campaign ready. The worker pages through running campaigns in creation order so older campaigns waiting on SMTP results cannot hide newer ones. The admin list retains all active campaigns alongside the newest 100 campaigns.
