...
- schema and metric-definition version `1`;
...
Snapshots do not contain UIDs, email addresses, provider lists, raw authentication timestamps, activity events, screen views, analytics identifiers, or heartbeat records. There is no historical backfill in v1; charts begin accumulating after the scheduler is released.
...
`getAdminDashboardHistory` accepts only `30`, `90`, or `365` days and defaults to `90` when the caller omits the range. It queries date-keyed documents inside the requested inclusive UTC interval, discards malformed or incompatible-version documents, and returns valid points oldest first.

The Admin Dashboard requests 365 days once per refresh and switches among 30-, 90-, and 365-day views locally. It renders three focused charts:

1. authentication activity for the rolling 24-hour, 7-day, and 30-day windows, with eligible-account percentages in tooltips;
2. stacked Free, Basic, and Pro users plus onboarding completion;
3. stacked Pro and Basic monthly/yearly cadence, adding unknown-cadence series only when unknown values exist.
...
The charts need eight snapshots in the selected range before rendering. Before then, the dashboard shows collection progress. This is expected immediately after release and is not an error.
...
- If a daily snapshot is missing, leave the gap visible. V1 has no operator backfill function.
