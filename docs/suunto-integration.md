...
The corresponding documented webhook notifications are `SUUNTO_247_ACTIVITY_CREATED` and `SUUNTO_247_RECOVERY_CREATED`. They are handled by the existing signed Suunto webhook endpoint. The notification payload is used only to identify bounded local-day refetch windows; raw Health samples from the webhook are never placed in Firestore, the queue, or Health collections. After signature and bounded-shape validation, the handler resolves exactly one UID through `suuntoHealthWebhookAccountBindings/{sha256(providerUserId)}`. That browser-inaccessible reverse binding is written atomically with new Suunto OAuth credentials and backfilled for active staged legacy connections by `scheduleSuuntoHealthSync`. The ingress transaction rechecks rollout, the exact credential and token-root generation, authoritative connection metadata, and account deletion before creating compact `suuntoHealthWebhookIngress`. It then acknowledges Suunto and a retryable Firestore trigger performs Cloud Tasks fan-out asynchronously. Signed payloads that are malformed, oversized, unknown, disconnected, deleting, or outside rollout are deliberate permanent drops and receive `2xx` without ingress persistence; authentication failures remain rejected, and transient identity/transaction failures return `5xx` so Suunto retries.
...
```text
signed webhook ─> active UID binding ─> durable compact ingress ─> retryable fan-out ─┐
...
The short-lived, server-only schema-v3 ingress contains the already-bound Firebase UID, notification type, provider account ID needed to revalidate the credential, captured credential and connection generations, bounded windows, receive time, processing status, and a seven-day TTL. The reverse binding stores only UID plus credential generation behind a one-way document key; clients cannot read or write it. OAuth is the only transfer authority, so the scheduled legacy backfill refuses to overwrite a binding owned by another UID. The create transaction rechecks the binding, exact token/root, service metadata, user root, and deletion tombstone. Unknown, non-rollout, disconnected, and deleting accounts therefore create no ingress row. Its document ID is a one-way digest of the authenticated raw notification, so exact retries collapse to the same durable create; the raw notification and samples are not retained. Queue creation is transactionally fenced to the same binding, token, root, and service-meta values, and the queue worker rejects a changed credential or connection generation before provider I/O. Later same-day Activity or Recovery notifications receive a fresh refetch identity. Repeated poll/webhook/backfill work still resolves to deterministic Health source identities, and the shared writer treats unchanged content as unchanged rather than creating duplicates. Provider source keys, account IDs, device source names, and revision tokens are hashed before Health persistence.
...
Ordinary Suunto disconnect retains imported Sleep and Health history but stops new work. Guarded token cleanup removes the matching reverse binding and recursively removes pending webhook ingress. Account deletion additionally queries and recursively deletes any reverse binding still owned by the UID, then removes the Suunto token subtree, every matching `suuntoHealthWebhookIngress`, `sleepSyncQueue`, and DLQ row (including `suunto_health_poll`), and recursively removes `sleepSessions`, Health source records, Health sample chunks, and both sync-state collections under `users/{uid}`. Binding removal is ownership-guarded and also generation-guarded where cleanup can race credential replacement, so delayed cleanup for an old owner cannot delete a binding transferred by newer OAuth. The ingress trigger rechecks the live kill switch, staged-user allowlist, exact token binding, connection state, and account-deletion guard before fan-out. Malformed, disabled, unbound, non-rollout, deleting, or lifecycle-skipped ingress is recursively deleted rather than marked processed; only successful fan-out retains a processed row until TTL. Successful completion uses update-only semantics, so concurrent cleanup cannot recreate a deleted ingress document. TTL is only a fallback for successfully processed or transiently stranded operational rows. Deletion-aware completion, retry, and DLQ transitions cannot recreate removed work.
...
3. Deploy Firestore Rules for the backend-only binding collection, then deploy the Suunto OAuth binding writer/cleanup, `scheduleSuuntoHealthSync`, the shared dispatcher/worker, and `fanOutSuuntoHealthWebhookIngress`. Verify the trigger is active.
4. Run the Suunto Health scheduler once, verify a binding exists for every staged active legacy connection, and confirm there are no `conflict` or `inactive` warnings. This seed avoids requiring users to reconnect.
5. Only after binding seeding, deploy the updated signed webhook handler and register Activity and Recovery webhook notifications with Suunto if they are not already registered. This ordering prevents permanent `2xx` drops while legacy bindings are absent and prevents the handler from accepting ingress before an asynchronous consumer exists.
6. Verify poll, webhook, history, 401 refresh, reconnect, disconnect, and account deletion for the staged account.
7. Monitor queue age/retries, binding conflicts, opaque error categories, Health sync timestamps, validation rejects, and provider rate-limit responses before widening the allowlist.
