...
Production setup must subscribe the two Health notification types to `receiveSuuntoAppSleepData` and keep Sleep, Workout, and Route notifications pointed at `receiveSuuntoAppSleepData`, `insertSuuntoAppActivityToQueue`, and `insertSuuntoAppRouteToQueue` respectively, all using the configured `SUUNTOAPP_NOTIFICATION_SECRET`. This repository change does not register provider webhooks or deploy Functions.
...
5. Only after binding seeding, deploy `receiveSuuntoAppSleepData`, `insertSuuntoAppActivityToQueue`, and `insertSuuntoAppRouteToQueue`, then register Activity and Recovery webhook notifications with Suunto if they are not already registered. This ordering prevents permanent `2xx` drops for Workout, Route, Sleep, or Health while bindings are absent and prevents the Health handler from accepting ingress before an asynchronous consumer exists.
