...
The short-lived, server-only schema-v4 ingress contains one already-bound Firebase UID, notification type, provider account ID needed to revalidate the credential, captured credential and connection generations, bounded windows, receive time, processing status, and a seven-day TTL. Each reverse binding stores only one UID plus its credential generation behind a document key made from a one-way digest of the provider account and UID together; clients cannot read or write it. Connecting the same Suunto account to another Firebase user adds an independent binding instead of transferring webhook ownership. The create transaction rechecks every candidate binding, exact token/root, service metadata, user root, and deletion tombstone. Unknown, non-rollout, disconnected, and deleting accounts therefore create no ingress row. Each ingress document ID is a one-way digest of the authenticated notification digest plus its bound UID, so exact retries collapse independently for every eligible connection; the raw notification and samples are not retained. Queue creation is transactionally fenced to the same per-user binding, token, root, and service-meta values, and the queue worker rejects a changed credential or connection generation before provider I/O. Later same-day Activity or Recovery notifications receive a fresh refetch identity. Repeated poll/webhook/backfill work still resolves to deterministic Health source identities, and the shared writer treats unchanged content as unchanged rather than creating duplicates. Provider source keys, account IDs, device source names, and revision tokens are hashed before Health persistence.
