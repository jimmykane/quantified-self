...
to its payload. Shared definitions keep the large `get_training_metric` schema and the complete 36-tool `tools/list`
...
inspects all advertised schemas, calls all 36 tools, and validates successful `structuredContent` with direct Ajv 8 and
