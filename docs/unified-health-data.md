...
Date controls remain local to the chart. On mobile (up to 720px), a compact Material range menu sits between the older/newer arrows; desktop retains the inline range toggle. The inspected dates and Today jump have their own full-width row below the metric heading. Only metric/range changes enter the existing account-settings write queue; source filters remain local and never represent a preferred source. Selection feedback uses the shared haptic service, without feedback for hydration or unchanged Apply.
