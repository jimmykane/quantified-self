...
Heart rate has a read-only long-range exception: the 90-day and 1-year explorer loads its existing sample chunks in successive windows of at most 31 days, with the same 8-chunk pages and **one cumulative** 256-chunk/100,000-point/16-MiB budget (not a fresh budget per window). It creates temporary daily `HealthMetricValue` entries before projection rather than drawing an intraday trace. No Function, provider request, Firestore schema/index/rule change, write, or migration is involved. Existing histories therefore work without waiting for a new sync. Other metrics and short-range summary-only consumers retain their previous read behavior.
