...
On mobile (up to 720px), a sticky **View options** action at the bottom of the metric explorer replaces the inline range toggle and source chips. Its standard Material bottom sheet drafts both selections, applies them together, and cancels without changing the workspace. The active range/source summary remains on the action; the date, older/newer arrows, and Today jump stay in the detail header. Desktop retains inline controls. Highlights are unaffected. Source choices come only from the selected metric's loaded providers; a single provider is attribution-only, and loading/empty windows still allow a range change. Loading-state drafts preserve the existing provider selection. Auth, metric, or window changes dismiss/invalidate an open draft. Only range changes enter the existing account-settings write queue; provider filters remain local. Selection feedback uses the shared haptic service, without feedback for hydration or unchanged Apply.
