...
users/{uid}/activities (activity VO2 max) ──┼─ queryActivityHealthRange
                                            └─ bounded identity-free observations
                                                        │
                                                        └─ /health and owner dashboard; no Health write
...
- `shared/activity-health.ts` and `functions/src/health/activity-query.ts` define the separate bounded workout-evidence projection used by Health and owner dashboard tiles; `activity-callable.ts` enforces Authentication and App Check.
...
Catalog availability is resolved separately across all stored Health history. `AppHealthService.loadAvailableMetricIds()` runs one owner-scoped, server-resolved indexed count capped at one match for each stable metric ID; it never downloads or scans the user's complete Health history. Sleep availability uses a one-document query against the normalized Sleep-session collection. The already bounded selected-window and fixed 30-day priority Sleep reads also keep HRV available when they contain average or overnight HRV but no standalone Health HRV record exists. Health and Sleep apply successful availability results independently: a pending, denied, or failed domain remains unfiltered so a transient read problem cannot create a false absence, while the other domain can still hide confirmed absences. A visible notice explains the degraded state after a denied or failed check. After the checks settle, an unavailable selected/default metric falls back in product priority order (Sleep, Heart rate, HRV) and then to the first displayed catalog metric. Provider sync timestamp advances re-run Health metric availability discovery.
...
The app's read-only sample viewer supports up to 90 days by loading existing chunks in successive windows of at most 31 days, with the same 8-chunk pages and **one cumulative** 256-chunk/100,000-point/16-MiB budget (not a fresh budget per window). Heart rate keeps a separate 1-year exception: it walks those same bounded windows and creates temporary daily `HealthMetricValue` entries before projection rather than drawing a year-long intraday trace. No Function, provider request, Firestore schema/index/rule change, write, or migration is involved. Existing histories therefore work without waiting for a new sync. The public callable and MCP sample-query limit remains 31 days.

`health-heart-rate-summary.helper.ts` only summarizes canonical recorded `activity_interval_average`, `activity_interval_minimum`, `activity_interval_maximum`, and `daily_15_second` HR series. Each source record, series, account, device, provider-local date and UTC offset remains independent. Means are unweighted arithmetic means of valid recorded samples, not averages of chunk averages or estimates for missing intervals. Identical timestamp duplicates count once; conflicting duplicates, missing chunk indices, and mixed parent revisions invalidate the affected derived group. Parent ownership, membership, provider/account, date/offset, bounds, and revision must match. A truncated read drops its boundary date and all later days from calculated results, preventing a partial page from becoming a misleading daily mean; the original provider summaries remain available. Normal source/day coverage is never upgraded by this calculation. QS-derived entries use existing canonical Sports Lib display conversion and explicit derived origin/calculation metadata, and are never persisted.

Provider-reported daily extrema and Garmin's rolling 7-day mean keep their own series. A calculated daily mean is added for Garmin's representative samples without duplicating its reported extrema. Suunto's mean/lowest/highest interval averages are explicitly named as such, never called true daily HR extremes. Optional recorded interval minima/maxima have separate calculated summaries. The explorer promotes the reading name to the chart heading with provider attribution underneath, explains the time periods/calculations, and identifies omitted sample-only providers even when another source has summary charts. Snapshot HR retains session-specific labels and is not treated as all-day HR. The existing sample-only empty state still handles a completely empty long-range view.
...
Weight reads `events.startDate + stats.Weight`. Sports Lib profile Weight is repeated on child activities, so querying the parent event emits at most one context observation per imported event. Legacy event roots can omit provider attribution; for those candidates the callable batch-reads the exact server-owned Garmin, Suunto, COROS, and Wahoo event-metadata documents. Exactly one provider match is required, provider-account identity contributes only to the one-way source key, and ambiguous or unknown provenance is omitted instead of guessed. This is labelled `workout_profile_context`: it is not a weigh-in and is shown only when the active provider-filtered window contains no real `body_weight` Health observation.
...
The authenticated `/health` route is client-rendered and `noindex`, with owner-scoped reads for every signed-in, onboarded user. All signed-in users see a **Health** item without a beta badge after Dashboard, Calendar, and Training in primary navigation; signed-out navigation does not expose it. There is no UID allowlist for workspace navigation. The Dashboard header does not duplicate the Health or Training navigation links. Provider connection and history-import plan checks are unchanged. Each selectable metric can also be added as an independently configured owner dashboard tile; the Health workspace remains the full explorer.

The workspace opens on Resting heart rate for the latest 30 days when that metric is available. Highlights keep the priority order Sleep, Heart rate, then HRV and keep differing source semantics separate. HRV is shown only when an individual nightly recovery series has at least three observations in the current 14-day highlight window. The workspace loads 73 days of summaries so every plotted highlight date, including the oldest one, can be graded using its own complete preceding 60-day personal-range window. The selected HRV explorer loads a separate bounded 60-day pre-window summary context and includes the same pre-window span in its Sleep read; this gives the first visible date a complete baseline without expanding the visible date range or loading sample chunks. Stable opaque series identities join that context to the visible provider/account/semantic series without exposing account keys. Multiple eligible nightly values on one calendar day reduce to a median before the baseline calculation, preventing duplicate summaries from giving one night extra weight. The reusable personal-range and chart-overlay helpers calculate and render the same model in Highlights and the explorer. The baseline is the mean and population standard deviation from distinct days. One standard deviation around the mean is the normal band, between one and two deviations is cautionary, and beyond two is far outside; distance is symmetric, so an unusually high HRV can also be cautionary. The headline status compares the current seven-day average, requiring at least three distinct recent nights, with today's rolling 60-day range. Separately, each plotted nightly value is graded against the 60-day range ending on that date. Dates without enough point-in-time baseline data remain neutral. Explicit ECharts line segments and points preserve those historical green/yellow/red/neutral statuses instead of being recolored from the latest headline; the tooltip names the status for the hovered night. The shaded band follows each observation's own rolling range rather than repeating the selected window's final range. The shared ECharts renderer uses an invisible lower-bound series plus a filled range-width series, on a separate daily timeline from the window start through its end, including actual observation timestamps. The band steps forward using only history at each timestamp and remains visible through missing readings while 14 distinct days remain in the trailing 60 days. Measurements remain gaps; baseline-building dates stay unshaded. Axis bounds include all displayed historical ranges, and tooltips show the range on the hovered date using Sports Lib formatting. Both the compact Highlight and full explorer use this same renderer; the existing baseline calculation and source separation are unchanged. This deliberately approximates Suunto's personal-range presentation without claiming its unpublished proprietary calculation, and it is not a medical interpretation. A lone sleep-derived HRV scalar already present in the Sleep highlight is not repeated in a mostly empty HRV card. Every baseline remains scoped to one provider, local account, aggregation, semantic variant, origin, recording method, and unit; values are never blended across those boundaries. The explorer groups the user's available historical metrics by the stable catalog categories; an empty current date window does not remove a Health metric that exists elsewhere in Health history. Its Suunto Stress state chart preserves the provider's documented category order and maps relaxing, active, passive, and stressful to the app's green, blue, gray, and red palette through a discrete ECharts data dimension. Line segments, points, axis labels, and tooltip markers use the same mapping, while category text and vertical position keep the state understandable without color. Average or overnight HRV found in the loaded selected or recent Sleep sessions can also surface HRV for legacy/unreferenced Sleep data. `/health` does not encode workspace state in query parameters. The selected metric and `today`, `14d`, `30d`, `90d`, or `1y` range are validated and stored together as the client-owned `appSettings.healthWorkspace` account preference; a missing or invalid metric falls back to Resting heart rate and a missing or invalid range falls back to 30 days. Detailed samples are loaded through 90 days by the app reader in shared-contract-sized 31-day Firestore windows under one cumulative 256-chunk, 100,000-point and 16 MiB budget. The 1-year choice remains available for daily-summary readings and the existing calculated Heart-rate daily view. When the active provider selection is sample-only, Health, dashboard tiles and the dashboard configurator disable 1 year and normalize an older saved 1-year selection to 90 days. This app-only aggregation does not change the MCP or callable representative-sample contract, which remains 31 days. Late settings hydration restores untouched selections without replacing a choice made in the current session, and queued writes are fenced to the signed-in UID. The older/newer window position and provider filters remain local to the current workspace session and reset on the next visit. Today is one calendar day and can be paged one day at a time.

Totals render as bars, scalar or point readings as lines/points, and categorical readings as stepped series. Health metric charts use the shared ECharts host, theme, tooltip, resize, and mobile-interaction stack; Sleep continues to reuse its normalized Sleep chart. Each chart point retains its source-recorded timezone offset. Axis labels select the nearest reading's offset and tooltips use the exact reading's offset, preventing a provider-local day from being presented as the preceding UTC day while still handling offset changes across travel or daylight-saving transitions. When no valid offset exists, the deterministic UTC fallback remains. Detailed chunks load for Today, 14-day, 30-day, and 90-day windows. The 1-year view uses stored summaries, plus the calculated Heart-rate daily projection where applicable; sample-only selections are capped at 90 days. Today combines the day's source-attributed summaries with any available intra-day chunks, but it does not imply continuous coverage where a provider supplies intermittent samples or daily summaries only. Coverage, freshness, device attribution, partial results, superseded revisions, conflicts, and safe sync state stay visible. The expandable source-observation table is the accessible textual equivalent of the charts.

The **Sources** action sits beside the Health title on mobile and desktop through the shared header's `pageHeaderTitleActions` slot. Its standard Material bottom sheet owns source selection, provider recency/status, and the Connectivity link; there is no separate source footer, inline source-chip row, or bottom View options control. The draft applies to Highlights and the metric explorer together, without blending series or changing data queries. Known sources accumulate from existing successful Health, Sleep, activity-context, and sync-state reads in an owner-scoped in-memory inventory. This keeps choices stable across metric/window navigation without a new inventory query; switching accounts clears both inventory and filters. A selected provider remains selected even when the current metric/window has no readings, with an explicit empty state and Show all sources escape. All sources is represented by an empty filter; a lone provider is attribution-only. An unchanged draft preserves the filter, Cancel changes nothing, and auth, metric, window changes, or teardown invalidate an open draft. Sources needing attention are summarized by a small header indicator and explained in the sheet.

Below the desktop metric-list breakpoint (980px), the metric heading itself is a Material button opening a **Choose metric** bottom sheet. There is no separate outlined select or duplicate heading. The sheet reuses the available category groups and metric icons, keeps availability reactive while open, and focuses/marks the current choice. Selection applies immediately through the existing saved-preference path; dismissal leaves it unchanged and restores focus to the title. Auth, metric/window changes, and teardown invalidate late selections. The desktop list and source selection are unchanged.

Date controls remain local to the chart. On mobile (up to 720px), a compact Material range menu sits between the older/newer arrows; desktop retains the inline range toggle. The inspected dates and Today jump have their own full-width row below the metric heading. Metric/range changes and explicit Highlight source choices enter the existing account-settings write queue; workspace source filters remain local and independent of those display choices. Selection feedback uses the shared haptic service, without feedback for hydration or unchanged Apply.

Highlights with multiple sources use standard Material tabs, keeping every source’s latest Sports Lib-formatted reading in its tab label while mounting one chart or Sleep summary at a time. Each of Sleep, Heart rate, and HRV stores its explicit selection under appSettings.healthWorkspace.highlightSources. The preference contains only a stable opaque provider/account view key, independent of timestamps, ordinal labels, units, or the currently selected semantic variant; it does not make that provider authoritative. Missing or filtered choices fall back for display without overwriting the saved key. No preference is written for hydration, automatic fallback, data reordering, or an unchanged tab. Per-highlight touched state protects local choices from late settings echoes while still hydrating untouched highlights. Writes merge only touched Highlight keys through the existing owner-scoped settings path and serialized metric/range queue, including UID fences and the existing save-failure/Retry notice. Sleep and HRV attribution and calculations, source filters, queries, ingestion, and backend schemas remain unchanged.

The **Today’s heart rate** highlight uses only canonical recorded-device all-day chunks: Garmin `daily_15_second` representative samples and Suunto `activity_interval_average` readings. It does not substitute scalar summaries, daily extrema, Garmin's rolling seven-day average, Sleep HR, resting HR, COROS HRV-associated HR, or snapshots. Its bounded sample read covers today and the adjacent provider calendar dates, then clips actual timestamps to the viewer's local midnight-to-midnight window (including DST). This avoids losing midnight readings when provider and viewer calendar dates differ. Each eligible account stays separate; the latest Sports Lib-formatted value is accompanied by its local recorded time and timezone, not a live claim. Providers without today's eligible points and the whole card when empty are hidden, including under source filtering. Existing saved-source fallback never overwrites the preference, and same-user background refreshes retain loaded content until replacement data resolves. The explorer's selected range and the Sleep/HRV highlight windows are unchanged. No ingestion, Firestore schema, Rules, or Functions changes are involved.

### Shared dashboard reads

`HealthMetricQueryService` wraps the existing bounded Health, Sleep and workout-evidence readers. Identical owner/metric/window/sample-mode requests share one promise. A priority queue permits at most three active requests; selected previews take precedence over waiting thumbnails and suggestions. Successful reads remain cached for 60 seconds, with at most 12 completed entries. Rejections are evicted. Closing the last interested preview cancels queued work; active reads finish without updating destroyed subscribers. Account changes clear caches and reject old-generation responses.

`DashboardHealthService` composes those results without writing Health data or triggering metric rebuilds. Visible thumbnails load lazily; only five suggestion candidates are considered when the Health browser opens, and pinning a single metric skips catalog evidence. HRV loads the same separate 60-day pre-window summary context as the explorer. Sleep references use the existing permitted metric mapping. Health and workout failures are retained independently so surviving readings remain renderable. Explicit limits, stale source revisions, sample-only long ranges and empty periods have separate explanations; examples never qualify a suggestion. The Health add/edit browser uses `dashboard-health-preview.helper.ts` for deterministic canonical examples, projected through the same Health renderer and user-unit conversion as real data. Example source keys, availability and notes never enter settings or discovery; saved tiles do not enable this fallback. The preview flag is local to library renderers.

The Health hub uses this reader for its selected metric, pre-window HRV context, selected Sleep window and workout evidence. Sleep requests depend on the time window rather than unrelated metric selections. Successful manual mutations and observed provider-sync advances invalidate the shared owner cache. Dashboard subscriptions refresh atomically, retaining their last chart until the new result arrives. If a source refresh fails, the adapter retains that source’s last successful result for the same metric/window, labels it as previous readings, and offers Retry. A successful empty response clears the old readings. The Health hub clears dashboard drafts and pending pin actions when the signed-in owner changes; scrolling and focus do not invalidate the cache or clear Timeline notes.
...
The shared Health projection appends samples to locally owned series arrays rather than copying the growing array
for every reading. It formats only the bounded observation-table page after sorting; all samples and the total
row count are preserved. Dashboard source and unit changes reuse the current bounded evidence without restarting
its subscription. Range changes and explicit invalidation retain the existing refresh and error semantics.
...
Sleep duration, Sleep score, average/minimum sleep heart rate, recorded resting heart rate, maximum sleep blood oxygen, and average sleep respiration also project directly from already-loaded normalized Sleep sessions in both Health and Dashboard. A separate Health reference is not required, so Suunto and Garmin sessions qualify alongside COROS references when those fields are actually recorded. Minimum heart rate is never substituted for resting heart rate; maximum oxygen is never presented as an average. Typed references win for their exact session/field and retain their existing semantics without duplicate points; missing-reference nights reuse a known matching account identity. Providers/accounts and nap/main-sleep readings remain separate, and missing or invalid values never become zero. The Health picker retains observed summary metric availability for the signed-in owner across empty windows, clearing it on account change. This is a read-time frontend projection with no new requests, ingestion, storage or MCP scope changes; HRV projection and personal-range calculations are unchanged.

The Health explorer includes those Sleep readings in provider availability. Its loading/empty state waits for all relevant sources, while a surviving Health, Sleep or workout series remains visible when another source is pending or fails, with a partial-source notice. Sleep-only summaries do not suppress the shorter-range notice for all-day sample sources without supported daily summaries. The shared `healthMetricUsesSleep` predicate follows the existing Sleep-reference metric mapping for both the dashboard reader and Health loading state.

The shared Sleep reader attaches the Health writer's existing owner/provider/account hash locally (once per account per shared read). This keeps summary source keys stable across windows with or without typed references. Main-sleep summaries sort before naps for initial selection; an explicit saved nap selection is preserved. It adds no network reads and does not mutate normalized sessions. If Web Crypto is unavailable, the projection retains distinct Sleep account identities and reuses a known typed-reference identity where available.
