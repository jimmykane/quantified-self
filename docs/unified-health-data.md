...
Catalog availability is resolved separately across all stored history. `AppHealthService.loadAvailableMetricIds()` runs one owner-scoped, server-only indexed count capped at one match for each stable metric ID; it never downloads or scans the user's complete Health history. Sleep availability uses a one-document query against the normalized Sleep-session collection. The workspace hides unavailable entries only after both checks complete successfully. While either check is pending, denied, or failed, it keeps the complete catalog visible so a transient read problem cannot create a false absence. When the selected/default metric is confirmed unavailable, selection falls back in product priority order (Sleep, Heart rate, HRV) and then to the first available catalog metric. Provider sync timestamp advances re-run Health metric availability discovery.
