...
- The dashboard Activity Calendar tile is full section width, shows the current month, and keeps a selected-day panel beside the grid on desktop or below it on mobile. New tiles use four columns. Existing owner tiles receive a one-time versioned size migration; later manual resizes are honored and all other tile settings are preserved.
...
`CalendarDayHealthService` reads only the selected date using the bounded Health/Sleep readers and existing derived-metric snapshots. The shared `CalendarDayContextComponent` renders that evidence in the dashboard tile, full Calendar, and Today sheet. Sleep and HRV require an exact local date match and name their source; a last-known reading is never substituted. Sleep shows its score when available, otherwise its recorded duration. Past readiness requires an exact stored daily score, while today uses the same live readiness inputs as the Today card. Recovery-left is present only for today. Each source has an independent empty or error state. Owner checks prevent private reads from shared dashboards; abort and date-key guards prevent stale results replacing a newer selection. No month-wide health query or new calculation is performed.
