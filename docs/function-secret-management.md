...
The default contributor workflow never uses provider credentials. Run the isolated stack from the repository root; it creates or repairs a missing/empty/all-blank `functions/.secret.local` from the sentinel template and otherwise rejects missing, extra, blank, or non-sentinel bindings.

```bash
npm start
```

Firebase's Functions emulator uses `.secret.local` for bound secret parameters. Missing or blank parameters can fall back to Secret Manager, which is why the isolated workflow uses explicit sentinels rather than empty values. Never replace them with development or production credentials in the default workflow: provider calls and background workers can still reach real external services. See [Isolated local development](local-development.md).

Maintainer testing against a provider requires a separate, reviewed command path and development-only provider accounts. Such a workflow may use an ignored local secret file, but it must not weaken or reuse `npm start`, and it must never use production credentials.
...
4. Add the name with value `LOCAL_EMULATOR_DISABLED` to `functions/.secret.local.example` and update this inventory.
