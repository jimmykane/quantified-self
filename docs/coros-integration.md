...
The supported operator replay command is `npm --prefix functions run backfill-coros-daily-health`. Its dedupe generation ensures an older sleep-only operator run does not suppress the Health replay. The completed one-off legacy Sleep-to-Health migration tooling has been retired after a full production dry run found no remaining candidates.
...
- Access/refresh tokens remain in the existing `COROSAPIAccessTokens` tree. The projection-aware frontend prefers the bounded owner-readable connection-account projection, containing the account identity and connection time needed by existing connection and route UX. During rollout it falls back to the legacy token read only when the projection field is absent; an explicit empty projection remains authoritative. OAuth exchange, refresh, disconnect, and projection writes use the Admin SDK. Browser writes to the token tree remain denied; the fallback and its legacy owner-read Rule are removed together after production backfill verification. The delivery callables and workers do not return credentials; their browser responses contain only bounded upload/status identifiers and messages.
