...
is available to eligible connected Pro users for both standalone workouts and plans; there is no per-UID backend allowlist
or browser **Coming soon** setup gate. Existing delivery records, retry, resume, copy status, plan configuration and Stop
sync remain available. This does not change COROS activity/history, Sleep, Health, FIT activity upload, activity sync or
route delivery. Connecting COROS alone never opts in, and implementation does not authorize a provider call or deployment.
