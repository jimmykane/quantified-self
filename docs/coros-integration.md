...
- Access/refresh tokens remain in the existing `COROSAPIAccessTokens` tree, and Firestore Rules deny every browser read and write of that root and token subtree. OAuth exchange, refresh, disconnect, and the bounded owner-readable connection-account projection use the Admin SDK. The projection contains only the account identity and connection time needed by existing connection and route UX; credentials and lifecycle generations remain backend-only. The delivery callables and workers do not return credentials; their browser responses contain only bounded upload/status identifiers and messages.
