...
- Access/refresh tokens remain in the existing `COROSAPIAccessTokens` tree. The projection-aware frontend reads only the bounded owner-readable connection-account projection, containing the account identity and connection time needed by existing connection and route UX. OAuth exchange, refresh, disconnect, and projection writes use the Admin SDK. Browser writes to the token tree remain denied; its legacy owner read stays temporarily available only through the projection rollout and is removed after production backfill verification. The delivery callables and workers do not return credentials; their browser responses contain only bounded upload/status identifiers and messages.
