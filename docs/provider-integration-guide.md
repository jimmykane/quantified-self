...
| COROS | `private-rollout` | The exact-UID runtime continues batching at most 30 dated workouts, retaining stable partner IDs, applying per-item deletion outcomes, and linking an exact returned `planWorkoutId`. New browser setup is temporarily hidden as **Coming soon** under #648, while existing enabled delivery, reconciliation, retained copy/status records and Stop sync continue. Public delivery and remote checking remain disabled. Entitlement, repeated-ID replacement, overlapping-window behavior, reschedule/delete, callback/history correlation, and app/watch behavior still require separately authorized live evidence. |
