...
Token-refresh lease contention is an expected queue deferral, not a provider failure: before the losing worker clears only the current item's dispatch marker in a deletion- and current-item-guarded transaction, it must durably enqueue one distinct delayed recovery task beyond the token-refresh lease. This prevents a concurrent dispatcher from mistaking the live original task for the follow-up and stranding the item after acknowledgement. If that enqueue cannot be confirmed, retain the original Cloud Task retry. A confirmed contention deferral does not consume the provider-failure retry budget or produce an error-level worker outcome. A genuine refresh failure remains on the normal retry/DLQ path.
