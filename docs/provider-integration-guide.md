...
| COROS    | Activity and daily sleep-summary import, activity upload, and activity sync to Suunto/Wahoo | Sleep polling runs every 24 hours over a rolling 7-day window. Connected Pro users can request a three-month sleep backfill in 30-day windows once every seven days; an admin script supports the same recovery range operationally. The endpoint supplies timing, average sleep HR, resting HR, and overnight HRV, but not sleep stages. |
| Wahoo    | Pro-only activity import through webhook/history, FIT activity delivery, direct GPX/FIT course/route delivery, and opt-in Suunto saved-route delivery | Wahoo imports only FIT-backed Wahoo-recorded workouts; retained Wahoo FITs can sync to Suunto, while Wahoo also accepts direct and selected source-event activity delivery, user-selected GPX/FIT course/route uploads, and Suunto routes already saved in Quantified Self. |
...
- Recheck connection, entitlement, deletion, and queue ownership before event persistence. If a provider supports mutable revisions, keep the active processing lease while recording a newer revision so two workers cannot interleave non-atomic event/activity writes. Make the lease longer than the worker runtime, recheck it immediately before persistence, and let the newer revision become dispatchable when the current worker completes. Stage original files outside user-readable paths until all deletion-guarded Firestore writes succeed. Providers that truly need atomic cross-account identity transfer must add a transfer-aware publication lease; the shared duplicate-token cleanup does not provide that guarantee.
...
- Persist a provider-issued asynchronous upload/job identifier as soon as it is issued. When initialization returns an ID before the provider accepts the blob, make the guarded state write succeed before sending any blob bytes; a state-write failure must abort the PUT so a task retry is safe. Subsequent retries must poll that identifier, not repost the original file, otherwise a timeout or post-provider write failure can create duplicate activities. Treat an HTTP/transport failure after an upload ID is issued as ambiguous acceptance and resume/poll the existing job rather than initializing a replacement. Direct callable upload flows must return opaque resume identifiers in retryable errors and accept them on the next attempt so browser retries follow the same job.
...
Apply the same typed failure contract to queued saved-route delivery. Provider adapters should map 429, transport failures, and ambiguous 5xx responses to retryable outcomes; explicit content or validation rejection to permanent outcomes; and auth/scope failures to skipped outcomes. Keep any provider job or external route identifier needed for idempotent resume/update behavior. When a provider lacks a stable create key or duplicate-reconciliation endpoint, retain one bounded queue retry policy, avoid adding a second adapter-level retry loop, and document the remaining duplicate risk.
