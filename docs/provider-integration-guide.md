...
Deploy callable Functions before the frontend that requires the completion receipt. Older frontends safely ignore the additive result, while a newer frontend deliberately rejects the former empty response.
...
- Give every new OAuth flow server-owned creation and expiry timestamps. The shared flow lifetime is one hour; token exchange must reject an expired context before calling the provider. The existing pending-disconnect scheduler also scans one bounded, document-ID-ordered page per provider, uses a durable cursor, and transactionally removes only expired OAuth fields or expired explicit-disconnect fences after rechecking the exact lifecycle generation, user-deletion guard, pending-disconnect state, and absence of token children. Empty provider roots are retained because legacy maintenance tooling can create a child without first generation-checking the root; document-only deletion could orphan that credential. Active flows, active leases, pending retries, token-bearing roots, changed generations, and missing/deleting users are preserved.
- Treat legacy OAuth roots without server-owned expiry as report-only until an explicit migration policy is approved. Inspect aggregate counts with `npm --prefix functions run audit-service-oauth-roots`; the command is bounded and read-only, rejects execution flags, and never emits user IDs or OAuth values. Any production cleanup or deployment still requires separate approval.
