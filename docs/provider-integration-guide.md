...
- Persist a provider-issued asynchronous upload/job identifier immediately after acceptance and before success metadata or queue-completion writes, even when the provider reports success in the same attempt. Subsequent retries must poll that identifier, not repost the original file, otherwise a timeout or post-provider write failure can create duplicate activities. Treat an HTTP/transport failure after an upload ID is issued as ambiguous acceptance: preserve the ID and resume/poll the existing job rather than initializing a replacement. If the provider accepted the upload but its resume state cannot be persisted, preserve the known identifiers in DLQ and fail closed instead of retrying the upload blindly.
