...
| Wahoo    | Pro-only activity import through webhook/history, FIT activity delivery, and direct FIT course/route delivery | Wahoo imports only FIT-backed Wahoo-recorded workouts; retained Wahoo FITs can sync to Suunto, while Wahoo also accepts direct and selected source-event activity delivery plus user-selected FIT course/route uploads. |
...
- A direct browser FIT upload is a separate product path. State whether it creates an event or route. Wahoo direct activity delivery intentionally sends to the provider only and retains only the short-lived browser row/upload token needed to show status. Wahoo direct course/route delivery accepts FIT course/route files only, makes a server-side idempotent route-library request, and does not create or retain a Quantified Self route.
