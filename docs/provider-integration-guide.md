...
- A documented accepted-but-pending job status is not a Cloud Task failure. Persist the job ID, consume a bounded polling budget, acknowledge the current task, and enqueue a status-only poll with the configured retry/backoff cadence. Emit an info-level structured poll-scheduled log. Scheduler or transport failures, exhausted polling, and terminal provider rejection must remain warning/error paths.
