...
- When an imported original file is published into a user-readable Storage path, transactionally recheck the current connection—including pending-disconnect state—and queue revision for every event/activity write and immediately before file promotion. Stage the file outside user-readable paths until those checks succeed. If a guard rejects after partial persistence, recursively remove only document roots created by that attempt. Providers that truly need atomic identity transfer may add a mapping-backed publication lease, but it is not the default model.
