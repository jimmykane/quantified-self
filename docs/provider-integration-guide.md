...
| --- | --- | --- |
| Garmin | `enabled` | Connected Pro users can explicitly deliver supported running/cycling workouts through separate Workout and Workout Schedule lifecycle records after granting `WORKOUT_IMPORT`. Those sub-sports still fold to broad `RUNNING`/`CYCLING`. The #733 pool-swim mapping is implemented with optional explicit pool length and swim-specific target-free steps, but new pool-swim delivery remains blocked pending separately authorized Garmin account proof and a readiness flip. Open-water swimming is unmapped. Synthetic fixtures and real Firestore transactions are not device evidence. There is no provider read/list path for authoritative missing-copy repair, and exact-profile device behavior remains unproven; completion correlation is #651. |
...
authored workout sport is never rewritten to make delivery pass. `Swimming` maps to `LAP_SWIMMING` only in the offline
...
and non-rest time steps must be 1–59 minutes. Garmin permits unspecified pools but older devices may not. Production
admission remains blocked until #733 account proof and a separate readiness flip; open water remains unsupported.
