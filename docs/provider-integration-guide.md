...
- While an explicit disconnect still owns a credential, OAuth start must return a retryable, user-visible `unavailable` response with bounded retry metadata. If a transaction proves that the disconnect has already removed every token, OAuth may rotate the flow generation and reclaim the empty lease immediately; all remaining disconnect cleanup must generation-check its writes and become stale.
- Emit structured disconnect lifecycle transitions for lease claim, cleanup completion or failure, finalization, and empty-lease recovery. Correlate transitions with a one-way digest of the random operation generation; never log the Firebase UID, provider account ID, token document ID, credential material, raw lifecycle generation, or provider error body.
