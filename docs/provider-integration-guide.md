...
| Wahoo    | Pro activity import, FIT activity delivery, direct/saved route delivery, and private Training delivery | Wahoo imports only FIT-backed Wahoo-recorded workouts; retained FITs can sync to Suunto/COROS. Private Training uses separate app-owned Plans and dated Workouts with a saved-zone seven-day horizon; exact returned Workout/Plan/token evidence can link the imported activity, while Wahoo-owned plans remain out of scope. |
...
| COROS | `private-rollout` | The exact-UID runtime continues batching at most 30 dated workouts, retaining stable partner IDs, applying per-item deletion outcomes, and linking an exact returned `planWorkoutId`. New browser plan-sync setup is temporarily hidden as **Coming soon** under #648, while standalone sends remain available and existing enabled plan delivery continues to follow its saved settings and plan status; retained copy/status records, retry and Stop sync remain available. Plan configuration and plan-workout resume stay hidden. Public delivery and remote checking remain disabled. Entitlement, repeated-ID replacement, overlapping-window behavior, reschedule/delete, callback/history correlation, and app/watch behavior still require separately authorized live evidence. |
| Wahoo | `private-rollout` | The exact-UID adapter delivers time-based Running/Cycling workouts as separate app-owned Plans and dated Workouts, with retained-ID recovery and independent Plan/Workout/association reads. The saved-zone window is today through today + 6; exact account-bound Workout/Plan/token evidence can link a returned recorded activity, while public delivery and automatic absence/repair remain disabled. Synthetic HTTP fixtures and real Firestore tests cover the implemented lifecycle; they do not establish deployment or device receipt. Production-account CRUD, reconnect, ownership, calendar-date and device acceptance remain pending in #649 using the existing production app/account; no sandbox is assumed. |
| Suunto | `private-rollout` | One workout maps to a dated SuuntoPlus Guide, not a native plan. The Guide recommends the exact canonical running/cycling profile selected in QS, including Trail Running, Treadmill, Mountain Biking, Indoor Cycling, E-Biking/E-MTB, and Hand Cycle where the documented Suunto activity catalog provides an ID. ZIP/icon CRUD and exact external-ID recovery reuse existing OAuth and the existing Suunto API subscription key with Guides access; today through today + 6 is a QS product window. Synthetic HTTP/FIT and real Firestore tests cover lifecycle and private completion evidence. Actual app/watch visibility remains unknowable through the partner API and is not claimed from fixtures or positive cloud reads. #710 keeps negative classification and automatic repair disabled. |
...
- When FIT-file inference is known to lose canonical activity types, correct the provider result only through an explicit, documented mapping. Wahoo activity-sync delivery reads the persisted Sports Lib `Activity Types` value, maps multiple canonical types to Wahoo `MULTISPORT`, and performs an idempotent workout `PUT` containing only the documented type field after Wahoo returns a workout ID; do not overwrite the user-visible workout title. An unmapped type deliberately keeps Wahoo's inference; never default it to cycling, `OTHER`, or an approximate type. Persist the asynchronous upload ID before retrying a failed correction so the FIT `POST` is never repeated. Direct browser uploads deliberately retain Wahoo's inferred type because they have no durable server-side activity record for an asynchronous correction.
