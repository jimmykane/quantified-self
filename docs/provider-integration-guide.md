...
| Wahoo | `private-rollout` | The exact-UID adapter delivers time-based Running/Cycling workouts as separate app-owned Plans and dated Workouts, with retained-ID recovery and independent Plan/Workout/association reads. The saved-zone window is today through today + 6; public delivery and automatic absence/repair remain disabled. Synthetic HTTP fixtures and real Firestore tests cover the implemented lifecycle; they do not establish deployment or device receipt. Production-account CRUD, reconnect, ownership, calendar-date and device acceptance remain pending in #649 using the existing production app/account; no sandbox is assumed. |
