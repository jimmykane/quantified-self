...
`shared/training-delivery-rollout.ts` contains no per-UID allowlist. The browser separately labels new COROS plan-sync
setup **Coming soon** while keeping standalone Send and existing plan records, Retry, copy status and Stop available.
