...
Token-refresh lease contention is an expected queue deferral, not a provider failure: the losing worker must clear only the current item's dispatch marker in a deletion- and current-item-guarded transaction, acknowledge its Cloud Task, and let the scheduled dispatcher redrive it after the winning refresh completes. It must not consume the provider-failure retry budget or produce an error-level worker outcome. A genuine refresh failure remains on the normal retry/DLQ path.
